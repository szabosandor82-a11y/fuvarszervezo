/* Fuvarszervező V44.2 – online többfelhasználós mobil felület.
   A hitelesítés, a fuvarok, átadások és szállítólevél-fotók Supabase-ben tárolódnak. */
(function (global) {
  'use strict';

  const VERSION = '44.2-online';
  const USERS = {
    ...
    'szabo.sandor82@gmail.com': { role: 'admin', displayName: 'Szabó Sándor', fixedPassword: '666' },
    'szabo.sandor@stand98.hu': { role: 'test', displayName: 'Teszt felhasználó', fixedPassword: '98' }
  };
  ...
  async function refreshTransfers() {
    ...
    catch (error) { console.warn('[V44.2] Átadások betöltési hibája', error); transferCache = []; }
  ...
  async function refreshOnlineNow() {
    ...
    } finally { suppressOnlineSave = false; refreshInProgress = false; }
  }
  global.refreshOnlineNow = refreshOnlineNow;

  function setLoginMode(mode) {
    loginMode = mode === 'signup' ? 'signup' : 'login';
    document.querySelectorAll('[data-auth-mode]').forEach(button => button.classList.toggle('active', button.dataset.authMode === loginMode));
    byId('authConfirmWrap')?.classList.toggle('hidden', loginMode !== 'signup');
    const hint = byId('authHint');
    if (hint) hint.textContent = loginMode === 'signup' ? 'Első belépés: állíts be jelszót. A sofőrök jelszava legalább 6 karakter legyen.' : 'Add meg az e-mail-címedet és a jelszavadat.';
    const submit = byId('authSubmit');
    if (submit) submit.textContent = loginMode === 'signup' ? 'Fiók létrehozása' : 'Belépés';
    authMessage('');
  }

  function backendPassword(email, enteredPassword) {
    const cfg = appUser(email);
    if (cfg?.role === 'admin' && enteredPassword === '666') return 'Fuvar-Admin-666!';
    if (cfg?.role === 'test' && enteredPassword === '98') return 'Fuvar-Test-98!';
  ...
  async function handleLogin(event) {
    ...
    const password = backendPassword(email, enteredPassword);
    ...
    if (loginMode === 'signup') {
      if (enteredPassword !== confirmation) return authMessage('A két jelszó nem egyezik.', true);
      if (cfg.fixedPassword && enteredPassword !== cfg.fixedPassword) return authMessage(`Ehhez a fiókhoz a megadott fix jelszót használd.`, true);
      if (!cfg.fixedPassword && enteredPassword.length < 6) return authMessage('A jelszó legalább 6 karakter legyen.', true);
    }
    const submit = byId('authSubmit');
    if (submit) submit.disabled = true;
    try {
      if (loginMode === 'signup') {
        const result = await global.V44Online.signUp(email, password);
        if (!result?.access_token) {
          authMessage('A fiók elkészült. Nyisd meg a megerősítő e-mailt, majd térj vissza és jelentkezz be.', false);
          setLoginMode('login');
          return;
        }
      } else await global.V44Online.signIn(email, password);
  ...
  function scheduleSync() {
    ...
    saveTimer = setTimeout(async () => {
      try {
        ...
        console.error('[V44.2] Online mentési hiba', error);
  ...
  function installGuardsAndHooks() {
    ...
    if (cameraForm) cameraForm.onsubmit = async event => {
      ...
      if (submit) { submit.disabled = true; submit.textContent = 'Feltöltés és e-mail küldése…'; }
      try {
        ...
        order.deliveryReports.push({ id: result.report.id, at: result.report.created_at || new Date().toISOString(), note: byId('cameraNote')?.value || '', photoCount: files.length, fileCount: result.report.fileCount, hasAudio: typeof audioBlob !== 'undefined' && !!audioBlob, createdBy: currentSession?.user?.email, emailStatus: result.emailResult?.ok === false ? 'failed' : 'sent' });
        cameraForm.closest('dialog')?.close();
        if (typeof audioBlob !== 'undefined') audioBlob = null;
        if (typeof save === 'function') save();
        const emailOk = result.emailResult?.ok !== false;
        alert(emailOk ? 'A szállítólevél online elmentve, és az automatikus e-mail elküldve.' : `A szállítólevél online elmentve, de az e-mail küldése nem sikerült: ${result.emailResult?.error || 'ismeretlen hiba'}`);
  ...
  async function renderAdminOnlinePage() {
    ...
    try {
      const [settings, transfers] = await Promise.all([global.V44Online.getSettings(), global.V44Online.listTransfers()]);
      transferCache = transfers;
      host.innerHTML = `<div class="panel online-settings-card"><h3>Szállítólevél e-mail beállítása</h3><label>Címzett e-mail<input id="deliveryEmailSetting" type="text" value="${safe(settings.delivery_email || '')}"></label><small>A címzettet később itt lehet módosítani. A fotó minden esetben a buborékban is megmarad.</small><div class="online-settings-actions"><button type="button" id="saveOnlineSettings">Mentés</button><button type="button" class="secondary" id="forceOnlineSync">Minden fuvar online mentése</button><button type="button" class="secondary" id="refreshOnlineAdmin">Online frissítés</button></div></div>
      ...
      byId('saveOnlineSettings')?.addEventListener('click', async () => {
        const delivery_email = byId('deliveryEmailSetting')?.value.trim();
        if (!delivery_email) return alert('Add meg a címzett e-mail-címet.');
        try { await global.V44Online.updateSettings({ delivery_email }); alert('A címzett e-mail elmentve.'); }
        catch (error) { alert(`Beállítási hiba: ${error.message}`); }
      });
  ...
  function injectOnlineUi() {
    ...
    if (!document.querySelector('[data-auth-mode="login"]')) {
      const modes = document.createElement('div'); modes.className = 'auth-mode-tabs';
      modes.innerHTML = '<button type="button" data-auth-mode="login" class="active">Belépés</button><button type="button" data-auth-mode="signup">Első belépés</button>';
      byId('authForm')?.prepend(modes);
      modes.querySelectorAll('[data-auth-mode]').forEach(button => button.addEventListener('click', () => setLoginMode(button.dataset.authMode)));
    }
  }

  function startPolling() {
    global.V44Online.startPolling(async orders => {
      if (!currentProfile || refreshInProgress) return;
      suppressOnlineSave = true;
      try {
        state.orders = orders;
        state.backlog = await global.V44Online.fetchBacklog();
        if (typeof save === 'function') save(false);
        await refreshTransfers();
        if (isAdmin()) { if (typeof render === 'function') render(); await renderAdminOnlinePage(); }
  ...
  async function restoreOnlineSession() {
    ...
    try {
      ...
      console.warn('[V44.2] Munkamenet visszaállítási hiba', error);
