  ...
  */
  ...
  async function roadMatrixV49(points) {
    ...
    try {
      ...
      console.warn('[V64] Közúti mátrix nem elérhető; légvonalas tartalék használata.', error);
    }
    return fallback;
  }
  ...
  */
  ...
  async function balanceActionV44() {
    try {
      ...
      alert(`Fuvarok V64 szerint szétosztva és felrakási sorrendbe rendezve.\n${result.summary}${conflictText}\nAzonos beszállító egy sofőrnél marad. A lerakók nem részei az optimalizálásnak.`);
      return result;
    } catch (error) {
      console.error('[V64] Szétosztási hiba', error);
  ...
  async function optimizeActionV44() {
    try {
      ...
      await persistOnlineV49();
      if (typeof render === 'function') render();
      alert('V64 optimalizálás elkészült: kizárólag a felrakók sorrendje változott. Lerakó és sofőr nem változott.');
      return true;
    } catch (error) {
      console.error('[V64] Optimalizálási hiba', error);
  ...
  */
  ...
  function bindV44() {
    ...
    if (balanceButton) {
      ...
      balanceButton.title = 'V64: sáv-alapú szétosztás; Márió=észak/kelet Pest, Patrik=közép/dél Pest és Buda, Martin=5-6 m szálanyag';
    }
    if (optimizeButton) {
      ...
      optimizeButton.title = 'V64: lakhely -> felrakók -> lerakók lánc optimalizálása, sofőrváltás nélkül';
    }
  ...
  global.V63Planner = global.V64Planner;
  global.V62Planner = global.V64Planner;
  global.V61Planner = global.V64Planner;
  global.V60Planner = global.V64Planner;
  global.V59Planner = global.V64Planner;
  global.V58Planner = global.V64Planner;
  global.V57Planner = global.V64Planner;
  global.V56Planner = global.V64Planner;
  global.V55Planner = global.V64Planner;
  global.V54Planner = global.V64Planner;
  global.V53Planner = global.V64Planner;
  global.V50Planner = global.V64Planner;
  global.V49Planner = global.V64Planner;
  global.V44Planner = global.V64Planner;
