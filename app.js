const KEY='fuvarszervezo_v11';const APP_VERSION=(()=>{const v=window.V95Planner?.version||window.V55Planner?.version||window.V54Planner?.version||window.V53Planner?.version||'';return v?('V'+v):'V55'})();const $=s=>document.querySelector(s),$$=s=>document.querySelectorAll(s);
...
function shiftWorkday(value,direction=1){
  const base=value instanceof Date?new Date(value):new Date(String(value)+'T12:00:00');
  if(isNaN(base))return'';
  const step=direction<0?-1:1;
  do{base.setDate(base.getDate()+step)}while(base.getDay()===0||base.getDay()===6);
...
function syncScheduleDate(){
  if(!syncDateParts('schedule',true))return false;
  const normalized=normalizeWorkday($('#scheduleDate').value);
  if(normalized&&normalized!==$('#scheduleDate').value)setScheduleParts(normalized);
  return true
}
...
function attachDataCombo(input, listId, itemsFn, onPick){
  ...
  input.addEventListener('keydown', event => {
    ...
    } else if(event.key === 'Tab' && !list.hidden && options.length){
      /* V78: a Tab is kitölti a mezőt – kiegészítésként. Ha nyíllal kijelöltél
         egy sort, azt veszi. Ha nem, de a szűkítés után CSAK EGY találat
         maradt, azt írja be: ilyenkor nincs mit eltéveszteni.
         Több találatnál nem választ, nehogy felülírja, amit gépeltél –
         ilyenkor a Tab a szokásos módon a következő mezőre lép. */
      if(active >= 0){ event.preventDefault(); pick(active); }
      else if(options.length === 1){ event.preventDefault(); pick(0); }
      else { list.hidden = true; }
    } else if(event.key === 'Escape'){ list.hidden = true; }
  });
...
function setupMasterCombos(){
  /* V78 – A NÉVMEZŐBE CSAK A CÉGNÉV KERÜL

     Eddig a felrakó mezőbe a "Cég · Cím" alak került, ezért a cím két helyen
     szerepelt, és a telephely megváltoztatásához a nevet is át kellett írni.
     Mostantól a névmező a cég neve, a cím pedig kizárólag a cím mezőben áll –
     ott szabadon választható a cég ÖSSZES telephelye közül. */
  attachDataCombo($('#supplierSearch'), 'supplierList',
    () => pickupTargetOptions().map(target => ({ label: target.label, note: target.hint,
      search: `${target.label} ${target.address || ''}`, ref: target })),
    item => {
      const ref = item.ref || {};
      // a mezőben a cég neve marad, cím nélkül
      const plainName = ref.kind === 'supplier' ? (ref.ref?.name || ref.label) : ref.label;
      if($('#supplierSearch')) $('#supplierSearch').value = String(plainName || '').split(' · ')[0];
      if($('#pickupAddress')) $('#pickupAddress').value = ref.address || '';
      fillSupplierAddressList($('#supplierSearch')?.value || '');
    });
...
/* V62 – a felrakó is lehet PROJEKT.

   Visszárunál és bérelt eszköznél a fuvar egy építkezésről indul, nem
   beszállítótól. Eddig a felrakó mezőben csak beszállítók szerepeltek, a
   lerakóban pedig már ott voltak a beszállítói telephelyek is – így a
...
window.pickupTargetOptions=pickupTargetOptions;
...
function fillSearchableMasters(){const sv=$('#supplierSearch')?.value||'',pv=$('#projectSearch')?.value||'';
  ...
  // V78: a javaslatlista cégneveket kínál, nem "cég · cím" párokat
  const uniqueSupplierNames=[...new Set(state.suppliers.filter(s=>s.active!==false&&s.name).map(s=>s.name))]
    .sort((a,b)=>a.localeCompare(b,'hu'));
  $('#supplierList').innerHTML=uniqueSupplierNames.map(name=>`<option value="${esc(name)}"></option>`).join('');$('#projectList').innerHTML=dropTargetOptions().map(t=>`<option value="${esc(t.label)}">${esc(t.hint)}</option>`).join('');if($('#supplierSearch'))$('#supplierSearch').value=sv;if($('#projectSearch'))$('#projectSearch').value=pv}
...
/* V63 – EGYETLEN EXPORT A NAPRA, A SAJÁT SABLON SZERINT

   Korábban sofőrönként külön fájl készült, 350 ms késleltetéssel: három
   sofőrnél három letöltés, mindegyik más oszlopszerkezettel, mint a
   "Sablon - szállítások.xlsx" Fuvarok munkalapja.

   Mostantól egy fájl, egy munkalap, pontosan a sablon oszlopaival:

     A Dátum   B Sorrend   C sofőr   D Felrakó   E Felrakó címe
...
function exportExcel(){
  ...
  const rows = exportRowsV63(date);
  if(!rows.length) return alert('Az aktuális napon nincs exportálható fuvar.');
  ...
  sheet['!cols'] = [{wch:11},{wch:8},{wch:9},{wch:20},{wch:32},{wch:13},{wch:9},{wch:22},{wch:6},{wch:32},{wch:12},{wch:34},{wch:26}];
...
window.failOrderToTomorrow=id=>{
  ...
  const target=shiftWorkday(o.scheduleDate,1);
...
(function installForwardMasterLearning(){
  ...
  window.V50MasterLearning={learnFromOrder,placePlannerControls};
  window.V49MasterLearning=window.V50MasterLearning;
  window.V48MasterLearning=window.V50MasterLearning;
  window.V47MasterLearning=window.V50MasterLearning;
})();
...
function renderRoutes(){
  ...
  if(document.getElementById('dailylist')?.classList.contains('active')&&typeof renderDailyListV93==='function')setTimeout(renderDailyListV93,50);
}
async function drawMap(id){
  const map=maps[id],pts=[],date=selectedDate();if(!map)return;
  const v=state.vehicles.find(x=>x.id===id),home=await vehicleHome(v||{});if(home)pts.push(home);
