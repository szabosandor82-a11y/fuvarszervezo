...
ok(html.includes('id="pickupAddress" list="supplierAddressList"') && app.includes('function fillSupplierAddressList'), 'buborék szerkesztésnél a beszállító összes törzs szerinti telephelye választható');
ok(app.includes('if(supplier&&sameSupplier&&sameOldPickup){') && !app.includes('if(supplier&&sameSupplier&&sameOldPickup&&sameProject(candidate,order)){'), 'kézi telephelyváltás azonos beszállító minden aznapi és későbbi fuvarjára továbbterjed');
...
ok(planner41.includes('Válassz címet a beszállítói törzsből') && planner41.includes('<datalist'), 'Outlook felrakócím csak a kiválasztott beszállító törzs szerinti telephelyei közül választható');
