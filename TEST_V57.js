...
(async () => {
  ...
  await test('A motor verziója V57', async () => {
    assert.match(fs.readFileSync(__dirname + '/planner-v44.js', 'utf8'), /const VERSION = '57'/);
  });
  ...
  await test('PRPR kiosztás: a fel- és lerakó együtt dönt', async () => {
    ...
    await c.V57Planner.distributeOrderSetV44(c.state.orders);
  ...
  await test('A 3 méteres menetesszár nem szálanyag, tehát nem kötelező Martin', async () => {
    const c = createContext();
    assert.equal(c.V57Planner.v53IsLongOrder({ items: [{ name: 'Niczuk Menetesszár M10x3000mm - (25/doboz)' }] }), false);
    assert.equal(c.V57Planner.v53IsLongOrder({ items: [{ name: 'Menetes szál M10x6000mm' }] }), true);
  });
  ...
  await test('Semmilyen fájl nem ír beégetett verziószámot a fejlécbe', async () => {
    ...
    for (const file of files) {
      ...
      lines.forEach((line, i) => {
        ...
        if (/Fuvarszervező\s*V5[0-6]\b/.test(line)) bad.push(`${file}:${i + 1}`);
        if (/APP_VERSION\s*=\s*['"]V5[0-6]['"]/.test(line)) bad.push(`${file}:${i + 1} APP_VERSION`);
  ...
  await test('A belépés utáni címfrissítés is a motor verzióját használja', async () => {
    ...
    assert.ok(auth.includes('V57Planner?.version'), 'nem a motorból olvassa a verziót');
  ...
  await test('A törzsadat exportálható data.js formátumban', async () => {
    const c = createContext();
    const result = c.V57Planner.exportMasterDataV55();
  ...
  await test('A térkép a felrakókat rajzolja, sorszámmal, a lakhely nélkül', async () => {
    ...
    await c.V57Planner.drawMapV49('m');
  ...
  await test('A kézi átrendezés a sequence sorrendjét adja, nem optimalizál újra', async () => {
    ...
    const events = await c.V57Planner.buildManualRouteV55(vehicle);
  ...
  await test('A frissítés nem írja felül a kézi sorrendet', async () => {
    ...
    await c.V57Planner.drawMapV49('m');            // csak rajzolás
