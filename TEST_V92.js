...
(async () => {
  ...
  await test('A motor verziója V92', async () => {
    assert.match(fs.readFileSync(__dirname + '/planner-v44.js', 'utf8'), /const VERSION = '92'/);
  });
  ...
  await test('PRPR kiosztás: a fel- és lerakó együtt dönt', async () => {
    ...
    await c.V92Planner.distributeOrderSetV44(c.state.orders);
  ...
  await test('A 3 méteres menetesszár nem szálanyag, tehát nem kötelező Martin', async () => {
    const c = createContext();
    assert.equal(c.V92Planner.v53IsLongOrder({ items: [{ name: 'Niczuk Menetesszár M10x3000mm - (25/doboz)' }] }), false);
    assert.equal(c.V92Planner.v53IsLongOrder({ items: [{ name: 'Menetes szál M10x6000mm' }] }), true);
  });
  ...
  await test('A belépés utáni címfrissítés is a motor verzióját használja', async () => {
    ...
    assert.ok(auth.includes('V92Planner?.version'), 'nem a motorból olvassa a verziót');
  ...
  await test('A törzsadat exportálható data.js formátumban', async () => {
    const c = createContext();
    const result = c.V92Planner.exportMasterDataV55();
  ...
  await test('A térkép a felrakókat rajzolja, sorszámmal, a lakhely nélkül', async () => {
    ...
    await c.V92Planner.drawMapV49('m');
  ...
  await test('A kézi átrendezés a sequence sorrendjét adja, nem optimalizál újra', async () => {
    ...
    const events = await c.V92Planner.buildManualRouteV55(vehicle);
  ...
  await test('A frissítés nem írja felül a kézi sorrendet', async () => {
    ...
    await c.V92Planner.drawMapV49('m');            // csak rajzolás
  ...
  await test('A 09.10-i valós nap sorrendje a kézi optimum', async () => {
    ...
    await c.V92Planner.buildRoutePlansV44();
  ...
  await test('A lánc-kereső hét megállóig pontos optimumot ad', async () => {
    const c = createContext();
    const chain = c.V92Planner.chainOrderV53;
  ...
  await test('Atrendezes utan a terkep szamozasa is frissul', async () => {
    ...
    const c = createContext();
    const P = c.V92Planner || c.V65Planner;
    c.state.vehicles = [{ id: 'm', driverName: 'Márió', active: true }];
    c.state.orders = [
  ...
  function companyMoveCtx() {
    const c = createContext();
    c.renderRoutes = () => {};
    c.state.vehicles = [
      { id: 'm', driverName: 'Márió', active: true },
      { id: 'p', driverName: 'Patrik', active: true },
      { id: 't', driverName: 'Martin', active: true }];
    const mk = (id, vehicleId, address, sequence, extra = {}) => ({
      id, scheduleDate: '2026-09-01', vehicleId, sequence, orderNo: '60' + id,
      pickupName: 'Lambda Systeme Kft.', pickupAddress: address,
      projectName: 'Cosmo', dropAddress: '', items: [], ...extra });
    c.state.orders = [
      mk('1', 'm', '1117 Budapest, Hengermalom út 47A', 1),
      mk('2', 'p', '1106 Budapest, Akna u. 2-4.', 1),
      mk('3', 't', '1117 Budapest, Hengermalom út 47A', 1),
      mk('4', 'm', 'x', 2, { pickupName: 'Más Cég Kft' })];
    return c;
  }

  await test('Egy beszallito OSSZES aznapi fuvara athelyezheto', async () => {
    const c = companyMoveCtx();
    const moved = c.moveCompanyOrdersToDriver('1', 't');
    assert.equal(moved, 3, 'nem mind a harom Lambda-fuvar mozgott: ' + moved);
    const lambda = c.state.orders.filter(o => /lambda/i.test(o.pickupName));
    assert.ok(lambda.every(o => o.vehicleId === 't'), 'maradt Lambda-fuvar masik soforNel');
    // MASIK ceg erintetlen
    const other = c.state.orders.find(o => o.pickupName === 'Más Cég Kft');
    assert.equal(other.vehicleId, 'm', 'a masik ceg fuvarjat is elvitte');
    // a sorszamok ujraszamozodnak
    const martin = c.state.orders.filter(o => o.vehicleId === 't').map(o => o.sequence).sort();
    assert.deepEqual(martin, [1, 2, 3], 'nem folytonos a sorszamozas: ' + martin.join(','));
  });

  await test('Az elintezett es kesz fuvarok a helyukon maradnak', async () => {
    const c = companyMoveCtx();
    c.state.orders.push({ id: '5', scheduleDate: '2026-09-01', vehicleId: 'm', sequence: 3,
      orderNo: '605', pickupName: 'Lambda Systeme Kft.', pickupAddress: 'Akna u. 2-4.',
      projectName: 'Cosmo', dropAddress: '', items: [], completed: true });
    const moved = c.moveCompanyOrdersToDriver('1', 't');
    assert.equal(moved, 3, 'a kesz fuvart is elvitte');
    assert.equal(c.state.orders.find(o => o.id === '5').vehicleId, 'm', 'a kesz fuvar sofore megvaltozott');
  });

  await test('A sofor-valaszto csak ott jelenik meg, ahol van ertelme', async () => {
    const v37 = fs.readFileSync(__dirname + '/planner-v37.js', 'utf8');
    assert.ok(v37.includes('function companyMoveControl'), 'nincs sofor-valaszto');
    const start = v37.indexOf('function companyMoveControl');
    const body = v37.slice(start, v37.indexOf('function companyOrdersOnDay'));
    assert.match(body, /related\.length < 2/, 'egyetlen fuvarnal is megjelenik');
    assert.ok(body.includes("vehicle.id !== currentVehicleId"), 'a sajat sofort is kinalja');
    assert.ok(v37.includes('companyMoveControl(group, vehicleId)'), 'a buborekra nincs kitéve');
    const css = fs.readFileSync(__dirname + '/styles.css', 'utf8');
    assert.ok(css.includes('select.company-move'), 'nincs stilusa a valasztonak');
  });

  if (!process.exitCode) console.log(`\nV92 elfogadási teszt: ${passed}/${total} sikeres.`);
})();
