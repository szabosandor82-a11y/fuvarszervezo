...
create table if not exists public.app_settings (
  ...
  delivery_email text not null default 'szabo.sandor@stand98',
  updated_at timestamptz not null default now(),
  updated_by text not null default public.current_email()
);
insert into public.app_settings(id,delivery_email) values(1,'szabo.sandor@stand98')
on conflict(id) do update set delivery_email=excluded.delivery_email,updated_at=now(),updated_by='v44.2-online-migration';
alter table public.app_settings enable row level security;
drop policy if exists settings_read on public.app_settings;
drop policy if exists settings_admin_write on public.app_settings;
create policy settings_read on public.app_settings for select to authenticated using (true);
create policy settings_admin_write on public.app_settings for all to authenticated using (public.current_role()='admin') with check (public.current_role()='admin');
...
-- Az Edge Function service_role kulccsal frissíti az email állapotát.
