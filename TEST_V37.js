...
(async()=>{
  ...
  await test('Régi csoportbontási jelölés mellett is együtt marad az azonos beszállító',async()=>{
    const c=createContext();
    ...
    c.state.orders=[a,b];
    const units=c.V37Planner.focusPickupUnits(c.state.orders);
    assert.equal(units.length,1);assert.equal(units[0].grouped,true);assert.equal(units[0].allPickupOrders.length,2);
  });

  await test('Azonos beszállító és azonos lerakó rendelései sem bonthatók szét',async()=>{
    const c=createContext();
