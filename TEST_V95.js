...
(async () => {
  ...
  await test('A motor verziója V95', async () => {
    assert.match(fs.readFileSync(__dirname + '/planner-v44.js', 'utf8'), /const VERSION = '95'/);
  });
  ...
  await test('PRPR kiosztás: a fel- és lerakó együtt dönt', async () => {
    ...
    await c.V95Planner.distributeOrderSetV44(c.state.orders);
  ...
  await test('A 3 méteres menetesszár nem szálanyag, tehát nem kötelező Martin', async () => {
    const c = createContext();
    assert.equal(c.V95Planner.v53IsLongOrder({ items: [{ name: 'Niczuk Menetesszár M10x3000mm - (25/doboz)' }] }), false);
    assert.equal(c.V95Planner.v53IsLongOrder({ items: [{ name: 'Menetes szál M10x6000mm' }] }), true);
  });
  ...
  await test('A belépés utáni címfrissítés is a motor verzióját használja', async () => {
    ...
    assert.ok(auth.includes('V95Planner?.version'), 'nem a motorból olvassa a verziót');
  ...
  await test('A törzsadat exportálható data.js formátumban', async () => {
    const c = createContext();
    const result = c.V95Planner.exportMasterDataV55();
  ...
  await test('A térkép a felrakókat rajzolja, sorszámmal, a lakhely nélkül', async () => {
    ...
    await c.V95Planner.drawMapV49('m');
  ...
  await test('A kézi átrendezés a sequence sorrendjét adja, nem optimalizál újra', async () => {
    ...
    const events = await c.V95Planner.buildManualRouteV55(vehicle);
  ...
  await test('A frissítés nem írja felül a kézi sorrendet', async () => {
    ...
    await c.V95Planner.drawMapV49('m');            // csak rajzolás
  ...
  await test('A 09.10-i valós nap sorrendje a kézi optimum', async () => {
    ...
    await c.V95Planner.buildRoutePlansV44();
  ...
  await test('A lánc-kereső hét megállóig pontos optimumot ad', async () => {
    const c = createContext();
    const chain = c.V95Planner.chainOrderV53;
  ...
  await test('Atrendezes utan a terkep szamozasa is frissul', async () => {
    ...
    const c = createContext();
    const P = c.V95Planner || c.V65Planner;
    c.state.vehicles = [{ id: 'm', driverName: 'Márió', active: true }];
    c.state.orders = [
  ...
  await test('A napi lista minden sofor oszlopat kirajzolja', async () => {
    ...
    assert.match(html, /3 fuvar · 5 tétel/, 'Marionál rossz az osszesites');
  });
  ...
  await test('A huzas ugyanazt a mozgatologikat hasznalja', async () => {
    ...
    assert.ok(html.includes('class="v56-index drag"'), 'a sorszam nem fogopont');
    assert.ok(html.includes('route-block'), 'a sor nem mozgathato elem');
  ...
  await test('A kesz fuvar jelolve van es nem mozgathato', async () => {
    ...
    assert.ok(doneRow.includes('v56-index drag'), 'a kesz fuvar nem mozgathato');
  });
  ...
  await test('A huzas athelyez es ujraszamoz', async () => {
    ...
    ['m', 't'].forEach(v => mk('v94-route-' + v));
    c.document = {
      querySelector: sel => nodes[sel] || null,
      querySelectorAll: sel => {
        const m = /^#(v94-route-[a-z]+) \.v93-row$/.exec(sel);
        return m ? nodes['#' + m[1]].rows : [];
      },
      getElementById: id => nodes['#' + id] || null };
    c.selectedDate = () => '2026-09-24';
    c.v24DriverKey = v => (v.driverName || '').toLowerCase().replace(/[^a-z]/g, '');
    c.activeVehicles = () => [{ id: 'm', driverName: 'Márió' }, { id: 't', driverName: 'Martin' }];
    const orders = [
      { id: '1', vehicleId: 'm', pickupName: 'Airvent', pickupAddress: 'Kőér u. 3B', sequence: 1, items: [1] },
      { id: '2', vehicleId: 'm', pickupName: 'Lambda', pickupAddress: 'Hengermalom 47A', sequence: 2, items: [1, 2] },
      { id: '3', vehicleId: 't', pickupName: 'Merkapt', pickupAddress: 'Maglódi 14B', sequence: 1, items: [1] }];
    c.state = { orders };
    c.dayOrders = id => orders.filter(o => o.vehicleId === id);
    c.isResolvedBacklogOrder = () => false;
    c.save = () => {}; c.renderRoutes = () => {}; c.render = () => {};
    const bound = [];
    c.Sortable = function (el, opt) { bound.push({ el, opt }); };
    vm.createContext(c);
    vm.runInContext(src, c, { filename: 'daily' });
    c.renderDailyListV93();

    assert.equal(bound.length, 2, 'nem kotott be minden oszlopot');
    const row = id => ({ dataset: { id }, querySelector: () => ({ textContent: '' }) });
    nodes['#v94-route-m'].rows = [row('1')];
    nodes['#v94-route-t'].rows = [row('3'), row('2')];
    bound[0].opt.onEnd({ item: { dataset: { id: '2' } }, to: { id: 'v94-route-t' } });

    const lambda = orders.find(o => o.id === '2');
    assert.equal(lambda.vehicleId, 't', 'nem kerult at a masik soforhoz');
    assert.equal(lambda.sequence, 2, 'rossz sorszamot kapott');
    assert.equal(orders.find(o => o.id === '3').sequence, 1, 'a masik sor nem szamozodott ujra');
    assert.equal(orders.find(o => o.id === '1').sequence, 1, 'a forras oszlop nem szamozodott ujra');
  });
  ...
  await test('MINDEN fogopont a soraban marad, nem ugrik ki', async () => {
    ...
    const rowClasses = ['v56-row', 'v93-row'];
  ...
  await test('A napi lista sorai ujjal is foghatok', async () => {
    const css = fs.readFileSync(__dirname + '/styles.css', 'utf8');
    const rule = css.match(/\.v93-row \.v56-index\.drag\{([^}]*)\}/);
  ...
  if (!process.exitCode) console.log(`\nV95 elfogadási teszt: ${passed}/${total} sikeres.`);
})();
