...
(async () => {
  ...
  await test('A motor verziója V93', async () => {
    assert.match(fs.readFileSync(__dirname + '/planner-v44.js', 'utf8'), /const VERSION = '93'/);
  });
  ...
  await test('PRPR kiosztás: a fel- és lerakó együtt dönt', async () => {
    ...
    await c.V93Planner.distributeOrderSetV44(c.state.orders);
  ...
  await test('A 3 méteres menetesszár nem szálanyag, tehát nem kötelező Martin', async () => {
    const c = createContext();
    assert.equal(c.V93Planner.v53IsLongOrder({ items: [{ name: 'Niczuk Menetesszár M10x3000mm - (25/doboz)' }] }), false);
    assert.equal(c.V93Planner.v53IsLongOrder({ items: [{ name: 'Menetes szál M10x6000mm' }] }), true);
  });
  ...
  await test('A belépés utáni címfrissítés is a motor verzióját használja', async () => {
    ...
    assert.ok(auth.includes('V93Planner?.version'), 'nem a motorból olvassa a verziót');
  ...
  await test('A törzsadat exportálható data.js formátumban', async () => {
    const c = createContext();
    const result = c.V93Planner.exportMasterDataV55();
  ...
  await test('A térkép a felrakókat rajzolja, sorszámmal, a lakhely nélkül', async () => {
    ...
    await c.V93Planner.drawMapV49('m');
  ...
  await test('A kézi átrendezés a sequence sorrendjét adja, nem optimalizál újra', async () => {
    ...
    const events = await c.V93Planner.buildManualRouteV55(vehicle);
  ...
  await test('A frissítés nem írja felül a kézi sorrendet', async () => {
    ...
    await c.V93Planner.drawMapV49('m');            // csak rajzolás
  ...
  await test('A 09.10-i valós nap sorrendje a kézi optimum', async () => {
    ...
    await c.V93Planner.buildRoutePlansV44();
  ...
  await test('A lánc-kereső hét megállóig pontos optimumot ad', async () => {
    const c = createContext();
    const chain = c.V93Planner.chainOrderV53;
  ...
  await test('Atrendezes utan a terkep szamozasa is frissul', async () => {
    ...
    const c = createContext();
    const P = c.V93Planner || c.V65Planner;
    c.state.vehicles = [{ id: 'm', driverName: 'Márió', active: true }];
    c.state.orders = [
  ...
  function dailyListCtx(orders, vehicles) {
    ...
    c.document = {
      querySelector: sel => sel === '#v93Board' ? c.board : (sel === '#v93Date' ? c.dateLabel : null),
      getElementById: () => null };
    c.selectedDate = () => '2026-09-24';
  ...
  await test('A huzas ugyanazt a mozgatologikat hasznalja', async () => {
    ...
    for (const id of ['route-m', 'route-p', 'route-t']) {
    ...
    assert.equal(c.sortableCalls, 1, 'nem indult el a mozgatologika');
  });
  ...
  await test('A kesz fuvar jelolve van es nem mozgathato', async () => {
    ...
    assert.match(html, /Kész · a helyén marad/, 'nincs magyarazat a kesz fuvaron');
  });
  ...
  if (!process.exitCode) console.log(`\nV93 elfogadási teszt: ${passed}/${total} sikeres.`);
})();
