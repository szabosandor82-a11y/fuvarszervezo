...
vm.runInThisContext(fs.readFileSync(__dirname + '/planner-v39.js','utf8'), {filename:'planner-v39.js'});
const V = global.V39OutlookImport;
...
test('KRPR: felrakó mindig a központi raktár', () => {
  ...
  eq(e.orderType,'KRPR'); eq(e.orderNo,'000745'); eq(e.pickupAddress,'2310 Szigetszentmiklós, Kereskedő utca 2.'); eq(e.projectName,'Budapest_LeJardin_II_felépítmény');
});
...
test('Szerelvénybolt címmigráció', () => { V.migrateV39MasterData(); eq(state.suppliers.find(x=>x.id==='s-szer').address,'1182 Budapest, Üllői út 807/B'); });
...
console.log(`\nV39 teszt: ${passed}/${passed+failed} sikeres.`);
