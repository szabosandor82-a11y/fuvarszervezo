...
(async () => {
  ...
  await test('A motor verziója V70', async () => {
    assert.match(fs.readFileSync(__dirname + '/planner-v44.js', 'utf8'), /const VERSION = '70'/);
  });
  ...
  await test('PRPR kiosztás: a fel- és lerakó együtt dönt', async () => {
    ...
    await c.V70Planner.distributeOrderSetV44(c.state.orders);
  ...
  await test('A 3 méteres menetesszár nem szálanyag, tehát nem kötelező Martin', async () => {
    const c = createContext();
    assert.equal(c.V70Planner.v53IsLongOrder({ items: [{ name: 'Niczuk Menetesszár M10x3000mm - (25/doboz)' }] }), false);
    assert.equal(c.V70Planner.v53IsLongOrder({ items: [{ name: 'Menetes szál M10x6000mm' }] }), true);
  });
  ...
  await test('A belépés utáni címfrissítés is a motor verzióját használja', async () => {
    ...
    assert.ok(auth.includes('V70Planner?.version'), 'nem a motorból olvassa a verziót');
  ...
  await test('A törzsadat exportálható data.js formátumban', async () => {
    const c = createContext();
    const result = c.V70Planner.exportMasterDataV55();
  ...
  await test('A térkép a felrakókat rajzolja, sorszámmal, a lakhely nélkül', async () => {
    ...
    await c.V70Planner.drawMapV49('m');
  ...
  await test('A kézi átrendezés a sequence sorrendjét adja, nem optimalizál újra', async () => {
    ...
    const events = await c.V70Planner.buildManualRouteV55(vehicle);
  ...
  await test('A frissítés nem írja felül a kézi sorrendet', async () => {
    ...
    await c.V70Planner.drawMapV49('m');            // csak rajzolás
  ...
  await test('A 09.10-i valós nap sorrendje a kézi optimum', async () => {
    ...
    await c.V70Planner.buildRoutePlansV44();
  ...
  await test('A lánc-kereső hét megállóig pontos optimumot ad', async () => {
    const c = createContext();
    const chain = c.V70Planner.chainOrderV53;
  ...
  await test('A BR0 a raktarkozi agon fut: felrako a kozponti raktar', async () => {
    const v41 = fs.readFileSync(__dirname + '/planner-v41.js', 'utf8');
    assert.ok(v41.includes("orderType === 'KRPR' || orderType === 'BR0'"),
      'a BR0 nem a raktarkozi agra fut');
    const c = createContext();
  ...
  await test('A kezi tetelszoveg mindket feluleten megjelenik', async () => {
    const auth = fs.readFileSync(__dirname + '/auth-v44-2.js', 'utf8');
    assert.ok(auth.includes('order.manualItems ?'), 'a sofori soron nem latszik');
  ...
  await test('A levelablak a mellekleteket es a kezi teteleket is mutatja', async () => {
    ...
    assert.ok(body.includes('target="_blank"'), 'a melleklet nem nyithato meg');
  });
  ...
  await test('A Nezet csuszkaja kulon rekeszben, a Csatolmany utan all', async () => {
    ...
    const html = c.V37Planner.groupedBubbles(c.state.orders, 'm', true);
    const order = [...html.matchAll(/class="(v56-index|v56-items-btn|v56-mail-btn|v56-drag-cell)"/g)].map(m => m[1]);
    assert.equal(order[order.length - 1], 'v56-drag-cell', 'a fogantyu nem a sor vegen all: ' + order.join(' → '));
    assert.ok(order.indexOf('v56-drag-cell') > order.indexOf('v56-mail-btn'), 'a fogantyu a Csatolmany elott van');
    const css = fs.readFileSync(__dirname + '/styles.css', 'utf8');
    const drag = css.match(/\.v56-drag\{[^}]*width:(\d+)px;height:(\d+)px/);
    assert.ok(drag && +drag[1] >= 30 && +drag[2] >= 32, 'a fogantyu tul kicsi: ' + (drag && drag[1] + 'x' + drag[2]));
    assert.ok(css.includes('.v56-drag-cell{'), 'nincs kulon rekesz a fogantyunak');
  });
  ...
  if (!process.exitCode) console.log(`\nV70 elfogadási teszt: ${passed}/${total} sikeres.`);
})();
