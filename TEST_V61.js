...
(async () => {
  ...
  await test('A motor verziója V61', async () => {
    assert.match(fs.readFileSync(__dirname + '/planner-v44.js', 'utf8'), /const VERSION = '61'/);
  });
  ...
  await test('PRPR kiosztás: a fel- és lerakó együtt dönt', async () => {
    ...
    await c.V61Planner.distributeOrderSetV44(c.state.orders);
  ...
  await test('A 3 méteres menetesszár nem szálanyag, tehát nem kötelező Martin', async () => {
    const c = createContext();
    assert.equal(c.V61Planner.v53IsLongOrder({ items: [{ name: 'Niczuk Menetesszár M10x3000mm - (25/doboz)' }] }), false);
    assert.equal(c.V61Planner.v53IsLongOrder({ items: [{ name: 'Menetes szál M10x6000mm' }] }), true);
  });
  ...
  await test('A belépés utáni címfrissítés is a motor verzióját használja', async () => {
    ...
    assert.ok(auth.includes('V61Planner?.version'), 'nem a motorból olvassa a verziót');
  ...
  await test('A törzsadat exportálható data.js formátumban', async () => {
    const c = createContext();
    const result = c.V61Planner.exportMasterDataV55();
  ...
  await test('A térkép a felrakókat rajzolja, sorszámmal, a lakhely nélkül', async () => {
    ...
    await c.V61Planner.drawMapV49('m');
  ...
  await test('A kézi átrendezés a sequence sorrendjét adja, nem optimalizál újra', async () => {
    ...
    const events = await c.V61Planner.buildManualRouteV55(vehicle);
  ...
  await test('A frissítés nem írja felül a kézi sorrendet', async () => {
    ...
    await c.V61Planner.drawMapV49('m');            // csak rajzolás
  ...
  await test('A 09.10-i valós nap sorrendje a kézi optimum', async () => {
    ...
    await c.V61Planner.buildRoutePlansV44();
  ...
  await test('A lánc-kereső hét megállóig pontos optimumot ad', async () => {
    const c = createContext();
    const chain = c.V61Planner.chainOrderV53;
  ...
  function selectCtx() {
    const src = fs.readFileSync(__dirname + '/app.js', 'utf8');
    const c = { console, Math, JSON, String, Number, Object, Array, RegExp, Error, WeakMap };
    c.globalThis = c; c.window = c; c.esc = v => String(v == null ? '' : v);
    c.norm = v => String(v || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[_.,;:()[\]{}\/\\-]+/g, ' ').replace(/\s+/g, ' ').trim();
    c.MutationObserver = undefined;
    c.createdInputs = [];
    c.document = {
      addEventListener() {},
      createElement: () => { const e = { type: '', className: '', placeholder: '', autocomplete: '', value: '', _h: {}, addEventListener(k, f) { e._h[k] = f; } }; c.createdInputs.push(e); return e; },
      querySelectorAll: () => []
    };
    vm.createContext(c);
    const i = src.indexOf('const SELECT_FILTER_MIN_OPTIONS');
    vm.runInContext(src.slice(i, src.indexOf('function makeSearchableSelects')), c, { filename: 'selfilter' });
    const j = src.indexOf('function supplierDisplay');
    vm.runInContext(src.slice(j, src.indexOf('\nfunction fillSearchableMasters')), c, { filename: 'masters' });
    return c;
  }
  const PROJECTS = [
    { id: 'p1', name: 'Budapest_Waterfront_City_V.ütem', address: '1033 Budapest, Folyamőr u. 9-11.' },
    { id: 'p2', name: 'Budapest_Waterfront_City_IV.ütem', address: '1033 Budapest, Folyamőr u. 9.' },
    { id: 'p3', name: 'Budapest_Cosmo_Residence', address: '1133 Budapest, Hegedűs Gyula utca 53.' },
    { id: 'p4', name: 'Budapest_Moxy_VUC', address: '1056 Budapest, Molnár utca 36.' }];

  await test('A szövegrészlet megtalálja a projektet a legördülőben', async () => {
    const c = selectCtx();
    c.state = { projects: PROJECTS, suppliers: [{ id: 's1', name: 'Székely Szerszám Kft', address: '2330 Dunaharaszti, Knézich utca 22' }] };
    assert.equal(c.findProjectByInput('waterfront'), null, 'két Waterfrontnál nem szabad tippelni');
    assert.equal(c.findProjectByInput('cosmo')?.name, 'Budapest_Cosmo_Residence');
    assert.equal(c.findProjectByInput('COSMO')?.name, 'Budapest_Cosmo_Residence', 'nem kis-nagybetű-érzéketlen');
    assert.equal(c.findProjectByInput('water iv')?.name, 'Budapest_Waterfront_City_IV.ütem', 'több szavas keresés nem megy');
    assert.equal(c.findProjectByInput('budapest'), null, 'négy találatnál is tippelt');
    assert.equal(c.findDropTargetByInput('szerszám')?.kind, 'supplier', 'a beszállító nem található részletre');
  });

  await test('A hosszú legördülők keresőmezőt kapnak és szűkíthetők', async () => {
    const c = selectCtx();
    const labels = ['', ...PROJECTS.map(p => p.name), 'Budapest_Kincsem_K6', 'Budapest_Sofitel_Hotel',
      'Budapest_LeJardin_II_felépítmény', 'Budapest_City_pearl_II.ütem'];
    const select = { multiple: false, dataset: {}, value: 'Budapest_Cosmo_Residence', innerHTML: '',
      options: labels.map(v => ({ value: v, textContent: v || 'Válassz…', attributes: [] })),
      parentNode: { insertBefore() {} }, focus() {} };
    c.makeSelectSearchable(select);
    assert.equal(select.dataset.searchable, '1', 'nem kapcsolt be a kereső');
    const box = c.createdInputs[0];
    assert.ok(box, 'nem jött létre keresőmező');
    const count = () => (select.innerHTML.match(/<option/g) || []).length;
    box.value = 'waterfront'; box._h.input();
    assert.equal(count(), 3, 'waterfront: ' + count() + ' elem (üres + 2 találat kell)');
    box.value = 'water iv'; box._h.input();
    assert.equal(count(), 2, 'több szavas szűrés: ' + count());
    box.value = 'zzz'; box._h.input();
    assert.equal(count(), labels.length, 'találat nélkül a teljes listát kell mutatni');
    box.value = ''; box._h.input();
    assert.equal(select.value, 'Budapest_Cosmo_Residence', 'elveszett a kiválasztott érték');
  });

  await test('A rövid legördülőket nem bonyolítjuk keresővel', async () => {
    const c = selectCtx();
    const select = { multiple: false, dataset: {}, value: '', innerHTML: '',
      options: [{ value: '', textContent: '', attributes: [] }, { value: 'm', textContent: 'Márió', attributes: [] },
        { value: 'p', textContent: 'Patrik', attributes: [] }],
      parentNode: { insertBefore() {} }, focus() {} };
    c.makeSelectSearchable(select);
    assert.notEqual(select.dataset.searchable, '1', 'három elemre is keresőt tett');
  });
  ...
  await test('Az újonnan megjelenő legördülők is bekapcsolódnak', async () => {
    ...
    const css = fs.readFileSync(__dirname + '/styles.css', 'utf8');
    assert.ok(css.includes('.select-filter'), 'nincs stílus a keresőmezőhöz');
  });

  if (!process.exitCode) console.log(`\nV61 elfogadási teszt: ${passed}/${total} sikeres.`);
})();
