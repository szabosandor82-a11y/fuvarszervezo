...
(async () => {
  ...
  await test('A motor verziója V88', async () => {
    assert.match(fs.readFileSync(__dirname + '/planner-v44.js', 'utf8'), /const VERSION = '88'/);
  });
  ...
  await test('PRPR kiosztás: a fel- és lerakó együtt dönt', async () => {
    ...
    await c.V88Planner.distributeOrderSetV44(c.state.orders);
  ...
  await test('A 3 méteres menetesszár nem szálanyag, tehát nem kötelező Martin', async () => {
    const c = createContext();
    assert.equal(c.V88Planner.v53IsLongOrder({ items: [{ name: 'Niczuk Menetesszár M10x3000mm - (25/doboz)' }] }), false);
    assert.equal(c.V88Planner.v53IsLongOrder({ items: [{ name: 'Menetes szál M10x6000mm' }] }), true);
  });
  ...
  await test('A belépés utáni címfrissítés is a motor verzióját használja', async () => {
    ...
    assert.ok(auth.includes('V88Planner?.version'), 'nem a motorból olvassa a verziót');
  ...
  await test('A törzsadat exportálható data.js formátumban', async () => {
    const c = createContext();
    const result = c.V88Planner.exportMasterDataV55();
  ...
  await test('A térkép a felrakókat rajzolja, sorszámmal, a lakhely nélkül', async () => {
    ...
    await c.V88Planner.drawMapV49('m');
  ...
  await test('A kézi átrendezés a sequence sorrendjét adja, nem optimalizál újra', async () => {
    ...
    const events = await c.V88Planner.buildManualRouteV55(vehicle);
  ...
  await test('A frissítés nem írja felül a kézi sorrendet', async () => {
    ...
    await c.V88Planner.drawMapV49('m');            // csak rajzolás
  ...
  await test('A 09.10-i valós nap sorrendje a kézi optimum', async () => {
    ...
    await c.V88Planner.buildRoutePlansV44();
  ...
  await test('A lánc-kereső hét megállóig pontos optimumot ad', async () => {
    const c = createContext();
    const chain = c.V88Planner.chainOrderV53;
  ...
  if (!process.exitCode) console.log(`\nV88 elfogadási teszt: ${passed}/${total} sikeres.`);
})();
