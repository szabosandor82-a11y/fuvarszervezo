...
(async () => {
  ...
  await test('A motor verziója V96', async () => {
    assert.match(fs.readFileSync(__dirname + '/planner-v44.js', 'utf8'), /const VERSION = '96'/);
  });
  ...
  await test('PRPR kiosztás: a fel- és lerakó együtt dönt', async () => {
    ...
    await c.V96Planner.distributeOrderSetV44(c.state.orders);
  ...
  await test('A 3 méteres menetesszár nem szálanyag, tehát nem kötelező Martin', async () => {
    const c = createContext();
    assert.equal(c.V96Planner.v53IsLongOrder({ items: [{ name: 'Niczuk Menetesszár M10x3000mm - (25/doboz)' }] }), false);
    assert.equal(c.V96Planner.v53IsLongOrder({ items: [{ name: 'Menetes szál M10x6000mm' }] }), true);
  });
  ...
  await test('A belépés utáni címfrissítés is a motor verzióját használja', async () => {
    ...
    assert.ok(auth.includes('V96Planner?.version'), 'nem a motorból olvassa a verziót');
  ...
  await test('A törzsadat exportálható data.js formátumban', async () => {
    const c = createContext();
    const result = c.V96Planner.exportMasterDataV55();
  ...
  await test('A térkép a felrakókat rajzolja, sorszámmal, a lakhely nélkül', async () => {
    ...
    await c.V96Planner.drawMapV49('m');
  ...
  await test('A kézi átrendezés a sequence sorrendjét adja, nem optimalizál újra', async () => {
    ...
    const events = await c.V96Planner.buildManualRouteV55(vehicle);
  ...
  await test('A frissítés nem írja felül a kézi sorrendet', async () => {
    ...
    await c.V96Planner.drawMapV49('m');            // csak rajzolás
  ...
  await test('A 09.10-i valós nap sorrendje a kézi optimum', async () => {
    ...
    await c.V96Planner.buildRoutePlansV44();
  ...
  await test('A lánc-kereső hét megállóig pontos optimumot ad', async () => {
    const c = createContext();
    const chain = c.V96Planner.chainOrderV53;
  ...
  await test('A Mentett fotok gomb jelzi, ha van feltoltott szallitolevel', async () => {
    ...
    const readBtn = order => {
      ...
      // a Mentett fotok gombot keressuk, nem az elso secondary gombot
      const m = html.match(/<button class="secondary[^>]*>([^<]*Mentett fotók[^<]*)<\/button>/);
      return { label: m ? m[1] : '', marked: /class="secondary has-photos"[^>]*>[^<]*Mentett fotók/.test(html) };
    };
  ...
  await test('A lerako cimet nem irja felul minden gepelesnel', async () => {
    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    const start = app.indexOf("$('#projectSearch').oninput");
    const body = app.slice(start, app.indexOf("$('#recipientId').onchange", start));
    assert.ok(body.includes('lastDropTargetKeyV83'), 'nincs cel-kovetes, a cim minden gepelesnel frissul');
    assert.ok(body.includes("if(targetChanged||!$('#dropAddress').value)$('#dropAddress').value=target.address"),
      'beszallitonal feltetel nelkul irja felul a cimet');
    assert.ok(body.includes("if(targetChanged||!$('#dropAddress').value)$('#dropAddress').value=p.address"),
      'projektnel feltetel nelkul irja felul a cimet');
    // a felrako oldalon nincs ilyen kezelo - ez volt az aszimmetria
    // a felrako oldalon UGYANEZ a szabaly ervenyes
    const pickupStart = app.indexOf("$('#supplierSearch').oninput");
    const pickupBody = app.slice(pickupStart, app.indexOf("$('#pickupAddress').onchange", pickupStart));
    assert.ok(pickupBody.includes('lastPickupTargetKeyV83'), 'a felrakonal nincs cel-kovetes');
    assert.ok(pickupBody.includes("if(pickupChanged||!$('#pickupAddress').value)"),
      'a felrako cimet is felulirja minden gepelesnel');
  });

  await test('A valasztott telephely megmarad, celvaltaskor frissul', async () => {
    let last = '';
    const handle = (target, current) => {
      const key = target ? `${target.kind}:${target.name}` : '';
      const changed = key !== last;
      last = key;
      return (changed || !current) ? (target?.address || '') : current;
    };
    const lambda = { kind: 'supplier', name: 'Lambda', address: 'Hengermalom út 47A' };
    const gienger = { kind: 'supplier', name: 'Gienger', address: 'Dűlő utca 31-35.' };

    let address = handle(lambda, '');
    assert.equal(address, 'Hengermalom út 47A', 'nem a kozpontot tolti ki elsore');
    address = 'Akna u. 2-4.';                       // a felhasznalo masik telephelyet valaszt
    address = handle(lambda, address);
    assert.equal(address, 'Akna u. 2-4.', 'a valasztott telephely elveszett gepeles kozben');
    address = handle(gienger, address);
    assert.equal(address, 'Dűlő utca 31-35.', 'celvaltaskor nem frissult a cim');
  });

  await test('Az import lerako listaja cegenkent egy elem, cim nelkul', async () => {
    const c = createContext();
    const html = c.V41OutlookImport.projectNameSelect({
      projectName: 'Budapest_Moxy_bérlemények', dropAddress: '1075 Budapest, Kazinczy u. 48.' });
    const labels = [...html.matchAll(/<option[^>]*>([^<]*)<\/option>/g)].map(m => m[1]);

    const lambda = labels.filter(l => /lambda/i.test(l));
    assert.equal(lambda.length, 1, 'a Lambda tobbszor szerepel: ' + lambda.join(' | '));
    assert.equal(lambda[0], 'Lambda Systeme Kft.', 'a cim is a nevben van: ' + lambda[0]);
    assert.ok(!/ · \d{4} /.test(lambda[0]), 'a cimke "ceg · cim" alaku');

    // a kivalasztott elem a fuvar sajat projektje
    const selected = [...html.matchAll(/<option[^>]*selected[^>]*>([^<]*)<\/option>/g)].map(m => m[1]);
    assert.deepEqual(selected, ['Budapest_Moxy_bérlemények'], 'nem a projekt van kivalasztva: ' + selected.join(', '));

    // a projekt cimkejeben sincs cim
    assert.ok(!labels.some(l => / · \d{4} Budapest/.test(l)), 'maradt cimet tartalmazo cimke');
  });

  await test('A lerako lista lenyegesen rovidebb lett', async () => {
    const c = createContext();
    const html = c.V41OutlookImport.projectNameSelect({ projectName: '', dropAddress: '' });
    const count = (html.match(/<option/g) || []).length;
    // korabban telephelyenkent listazott (527 elem), most cegenkent
    assert.ok(count < 450, 'nem rovidult a lista: ' + count);
    assert.ok(count > 300, 'tul sok elem tunt el: ' + count);
    const values = [...html.matchAll(/<option value="([^"]*)"/g)].map(m => m[1]).filter(Boolean);
    assert.equal(values.length, new Set(values).size, 'van ismetlodo ertek a listaban');
  });

  await test('Az importban a lerako cime is valaszthato', async () => {
    const c = createContext();
    c.state.orders = [{ projectName: 'Budapest_Moxy_bérlemények', dropAddress: '1061 Budapest, Király utca 12.' }];
    const read = (name, address) => {
      const html = c.V41OutlookImport.dropAddressSelect({ _id: 'e1', projectName: name, dropAddress: address });
      return [...html.matchAll(/<option value="([^"]*)">([^<]*)<\/option>/g)].map(m => ({ address: m[1], note: m[2] }));
    };

    // VISSZARU: a beszallito minden telephelye, kozpont elol
    const lambda = read('Lambda Systeme Kft.', '1117 Budapest, Hengermalom út 47A');
    assert.ok(lambda.length >= 2, 'nem kinalja mindket Lambda telephelyet: ' + lambda.length);
    assert.match(lambda[0].address, /Hengermalom/, 'nem a kozpont all elol');
    assert.equal(lambda[0].note, 'Központ');
    assert.ok(lambda.some(r => /Akna/.test(r.address)), 'az Akna utca nem valaszthato');

    // PROJEKT: torzsadat + korabbi fuvarok
    const moxy = read('Budapest_Moxy_bérlemények', '');
    assert.ok(moxy.some(r => /Kazinczy/.test(r.address) && r.note === 'törzsadat'), 'hianyzik a torzsadat cime');
    assert.ok(moxy.some(r => /Király/.test(r.address) && /korábbi fuvar/.test(r.note)),
      'hianyzik a korabbi fuvarbol ismert cim');
  });

  await test('A lerako cim mezoje kereso lenyilot kap az importban', async () => {
    const v41 = fs.readFileSync(__dirname + '/planner-v41.js', 'utf8');
    assert.ok(v41.includes('function dropAddressSelect'), 'nincs lerako cim-lenyilo');
    assert.ok(v41.includes('data-kind="drop-address"'), 'a mezo nincs megjelolve');
    assert.ok(!/data-field="dropAddress" value="\$\{htmlEsc\(entry\.dropAddress\)\}"/.test(v41),
      'maradt a regi, lenyilo nelkuli szovegmezo');
    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    assert.ok(app.includes('input[data-kind="drop-address"]:not([data-combo])'),
      'a kereso lenyilo nincs rakotve a lerako cim mezojere');
  });

  function returnCtx() {
    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    const c = { console, String, Object, Array, RegExp, Error, Math, Set, Map, JSON, setTimeout: f => f() };
    c.globalThis = c; c.window = c;
    c.norm = v => String(v || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()
      .replace(/[_.,;:()[\]{}\/\\-]+/g, ' ').replace(/\s+/g, ' ').trim();
    let n = 0; c.uid = () => 'i' + (++n);
    c.selectedDate = () => '2026-09-24';
    c.opened = null; c.alerted = '';
    c.openOrder = d => { c.opened = d; };
    c.alert = m => { c.alerted = m; };
    vm.runInNewContext(fs.readFileSync(__dirname + '/data.js', 'utf8'), c);
    c.state = { suppliers: c.SEED_DATA.suppliers.map((s2, i) => ({ ...s2, id: 's' + i })), orders: [] };
    vm.createContext(c);
    const i = app.indexOf('function returnOrder');
    vm.runInContext(app.slice(i, app.indexOf('window.returnOrder=returnOrder;')), c, { filename: 'return' });
    return c;
  }

  await test('A visszaru megforditja a fuvar iranyat', async () => {
    const c = returnCtx();
    c.state.orders = [{ id: 'o1', orderNo: '006327', vehicleId: 'martin',
      pickupName: 'Lambda Systeme Kft.', pickupAddress: '1117 Budapest, Hengermalom út 47A',
      projectName: 'Budapest_Moxy_bérlemények', dropAddress: '1075 Budapest, Kazinczy u. 48.',
      items: [{ _id: 'a', name: 'MEZ-keret', qty: 40, unit: 'm', received: true }] }];
    c.returnOrder('o1');
    const draft = c.opened;
    assert.ok(draft, 'nem nyilt meg az uj fuvar');
    // az irany megfordul
    assert.equal(draft.pickupName, 'Budapest_Moxy_bérlemények', 'a felrako nem a projekt lett');
    assert.equal(draft.pickupAddress, '1075 Budapest, Kazinczy u. 48.', 'a felrako cime nem a lerako cime lett');
    assert.equal(draft.projectName, 'Lambda Systeme Kft.', 'a lerako nem a beszallito lett');
    assert.match(draft.dropAddress, /Hengermalom/, 'a lerako cime nem a kozpont');
    assert.equal(draft.isReturn, true, 'nincs visszaru jelolve');
    assert.equal(draft.returnOfOrderId, 'o1', 'nincs meg az eredeti fuvar hivatkozasa');
  });

  await test('A visszaru gyujtokodot kap es a tetelek atjonnek', async () => {
    const c = returnCtx();
    c.state.orders = [{ id: 'o1', orderNo: '006327', pickupName: 'Lambda Systeme Kft.',
      pickupAddress: '1117 Budapest, Hengermalom út 47A', projectName: 'Budapest_Moxy_bérlemények',
      dropAddress: '1075 Budapest, Kazinczy u. 48.',
      items: [{ _id: 'a', name: 'MEZ-keret', qty: 40, unit: 'm', received: true, missingQty: 2 }] }];
    c.returnOrder('o1');
    const draft = c.opened;
    assert.equal(draft.orderNo, '000000', 'nem a gyujtokodot kapja');
    assert.equal(draft.items.length, 1, 'elvesztek a tetelek');
    assert.equal(draft.items[0].received, false, 'a tetel atvettkent jott at');
    assert.equal(draft.items[0].missingQty, '', 'a hianyzo mennyiseg atjott');
    assert.notEqual(draft.items[0]._id, 'a', 'a tetel ugyanazt az azonositot kapta');
    assert.match(draft.note || '', /Visszáru/, 'nincs magyarazo megjegyzes');
  });

  await test('Tobb telephelynel szol, hogy valassz', async () => {
    const c = returnCtx();
    c.state.orders = [{ id: 'o1', orderNo: '1', pickupName: 'Lambda Systeme Kft.',
      pickupAddress: 'x', projectName: 'P', dropAddress: 'y', items: [] }];
    c.returnOrder('o1');
    assert.match(c.alerted, /telephellyel szerepel/, 'nem szol a tobb telephelyrol');
    assert.match(c.alerted, /Lerakó címe/, 'nem mondja meg, hol lehet valasztani');
    // egytelephelyes cegnel ne zavarjon
    const single = returnCtx();
    const only = single.state.suppliers.find(su =>
      single.state.suppliers.filter(x => x.name === su.name).length === 1);
    single.state.orders = [{ id: 'o2', orderNo: '1', pickupName: only.name,
      pickupAddress: only.address, projectName: 'P', dropAddress: 'y', items: [] }];
    single.returnOrder('o2');
    assert.equal(single.alerted, '', 'egytelephelyes cegnel is figyelmeztet');
  });

  await test('A buborekban ott a Visszaru gomb', async () => {
    const v37 = fs.readFileSync(__dirname + '/planner-v37.js', 'utf8');
    assert.ok(v37.includes("returnOrder('"), 'nincs visszaru gomb a buborekon');
    assert.match(v37, /Visszaszállítás: a projekt lesz a felrakó/, 'nincs magyarazo sugo');
  });

  await test('A felrako cime PROJEKTNEL is valaszthato', async () => {
    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    const start = app.indexOf("attachDataCombo($('#pickupAddress')");
    const body = app.slice(start, start + 2200);
    assert.ok(body.includes('(state.projects || []).find'), 'a felrako cim listaja nem nezi a projekteket');
    assert.match(body, /push\(project\.address, 'törzsadat'\)/, 'a projekt cime nem kerul a listaba');
    assert.match(body, /korábbi fuvar · \$\{count\}×/, 'a korabbi fuvarok cimei nem kerulnek a listaba');
    ...
    c.state = { projects: c.SEED_DATA.projects.map((p2, k) => ({ ...p2, id: 'p' + k })),
      suppliers: c.SEED_DATA.suppliers.map((s2, k) => ({ ...s2, id: 's' + k })),
      orders: [{ projectName: 'Budapest_Moxy_bérlemények', dropAddress: '1061 Budapest, Király utca 12.' }] };
    ...
    const options = name => {
      ...
      const push = a => { const k = c.norm(a); if (!a || seen.has(k)) return; seen.add(k); rows.push(a); };
      if (project.address) push(project.address);
      const counts = new Map();
      for (const o of c.state.orders) {
        if (c.norm(o.projectName || '') !== key && c.norm(o.pickupName || '') !== key) continue;
        for (const a of [o.dropAddress, o.pickupAddress]) {
    ...
    const moxy = options('Budapest_Moxy_bérlemények');
    assert.ok(moxy.length >= 2, 'nem kinal cimet a projekthez: ' + moxy.length);
    assert.match(moxy[0], /Kazinczy/, 'nem a torzsadat cime all elol');
    assert.ok(moxy.some(a => /Király/.test(a)), 'a korabbi fuvar cime kimaradt');
  });

  await test('Az urlapnyitas nullazza a cel-koveto kulcsokat', async () => {
    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    const start = app.indexOf('function openOrder(o={})');
    const body = app.slice(start, start + 400);
    assert.ok(body.includes("lastPickupTargetKeyV83='';lastDropTargetKeyV83='';"),
      'az urlapnyitas nem nullazza a cel-koveto kulcsokat');
  });

  await test('Megforditott iranynal az import mindket cimet kinalja', async () => {
    const c = createContext();
    const read = html => [...html.matchAll(/<option value="([^"]*)">([^<]*)<\/option>/g)]
      .map(m => ({ address: m[1], note: m[2] }));

    // FELRAKO = projekt (visszaru forrasa)
    const pickup = read(c.V41OutlookImport.supplierAddressSelect({
      _id: 'e1', pickupName: 'Budapest_Moxy_bérlemények', pickupAddress: '' }));
    assert.ok(pickup.length >= 1, 'projekt felrakonal ures a cim lista');
    assert.match(pickup[0].address, /Kazinczy/, 'nem a projekt cimet kinalja');
    assert.equal(pickup[0].note, 'törzsadat');

    // LERAKO = beszallito (visszaru celja)
    const drop = read(c.V41OutlookImport.dropAddressSelect({
      _id: 'e1', projectName: 'Lambda Systeme Kft.', dropAddress: '' }));
    assert.ok(drop.length >= 2, 'nem kinalja a Lambda telephelyeit: ' + drop.length);
    assert.match(drop[0].address, /Hengermalom/, 'nem a kozpont all elol');
    assert.ok(drop.some(r => /Akna/.test(r.address)), 'az Akna utca nem valaszthato');
  });

  await test('A nev valtasakor a cim is kovetkezik', async () => {
    const v41 = fs.readFileSync(__dirname + '/planner-v41.js', 'utf8');
    // felrako -> projekt
    const pickupStart = v41.indexOf("input.dataset.kind === 'supplier-name'");
    const pickupBody = v41.slice(pickupStart, v41.indexOf("input.dataset.kind === 'supplier-address'", pickupStart));
    assert.ok(pickupBody.includes('const asProject ='), 'a felrako nevvaltasa nem kezeli a projektet');
    assert.ok(pickupBody.includes('entry.pickupAddress = asProject.address'), 'nem tolti be a projekt cimet');
    // lerako -> beszallito
    const dropStart = v41.indexOf("input.dataset.kind === 'project-name'");
    const dropBody = v41.slice(dropStart, dropStart + 1600);
    assert.ok(dropBody.includes('const company ='), 'a lerako nevvaltasa nem kezeli a beszallitot');
    assert.ok(dropBody.includes('entry.isReturn = true'), 'nem jeloli visszarunak');
    assert.match(dropBody, /entry\.dropAddress = selected\?\.dataset\.supplierAddress \|\| central\.address/,
      'nem a beszallito cimere all');
  });

  await test('Az import kezeloi csak LETEZO fuggvenyeket hivnak', async () => {
    /* A V87-ben egy nem letezo renderPreview() hivas allt a felrako
       nevvaltasanak agaban. A kezelo hibara futott, megszakadt, es a kartya
       nem frissult - ezert tunt ugy, hogy "semmi nem valtozik".
       Ez a teszt a modul OSSZES sajat fuggvenyhivasat vegignezi. */
    const v41 = fs.readFileSync(__dirname + '/planner-v41.js', 'utf8');
    const defined = new Set([...v41.matchAll(/function ([A-Za-z_$][\w$]*)\s*\(/g)].map(m => m[1]));
    // nyilfuggvenyek es helyben deklaralt segedek
    for (const m of v41.matchAll(/const ([A-Za-z_$][\w$]*)\s*=/g)) defined.add(m[1]);
    // metodushivasok (Object.assign, tomb-metodusok) nem sajat fuggvenyek
    for (const m of v41.matchAll(/\.([A-Za-z_$][\w$]*)\s*\(/g)) defined.add(m[1]);

    // a kezelo torzse, ahol a hiba volt
    const start = v41.indexOf("input.dataset.kind === 'supplier-name'");
    const body = v41.slice(start, v41.indexOf('entry.sourceOrderNos = orderNumbersOf', start));
    const code = body.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/[^\n]*/g, '');
    const called = [...code.matchAll(/\b([A-Za-z_$][\w$]*)\s*\(/g)].map(m => m[1]);
    const builtins = new Set(['if', 'for', 'while', 'switch', 'catch', 'return', 'typeof', 'find',
      'filter', 'map', 'includes', 'some', 'every', 'sort', 'concat', 'join', 'split', 'replace',
      'trim', 'push', 'slice', 'test', 'match', 'String', 'Number', 'Boolean', 'Object', 'Array',
      'Set', 'Map', 'Date', 'JSON', 'parseInt', 'parseFloat', 'nrm', 'htmlEsc', 'id']);
    const missing = [...new Set(called)].filter(name => !builtins.has(name) && !defined.has(name));
    assert.deepEqual(missing, [], 'nem letezo fuggvenyt hiv: ' + missing.join(', '));
  });

  await test('A felrako projektre valtasa kitolti a cimet', async () => {
    const c = createContext();
    const entry = { _id: 'e1', pickupName: 'Lambda Systeme Kft.',
      pickupAddress: '1117 Budapest, Hengermalom út 47A', warnings: [] };
    // a kezelo logikaja: projektre valtunk
    entry.pickupName = 'Budapest_Moxy_bérlemények';
    const asProject = c.state.projects.find(p2 => c.norm ? false : false) ||
      c.SEED_DATA.projects.find(p2 => p2.name === 'Budapest_Moxy_bérlemények');
    assert.ok(asProject?.address, 'a projektnek nincs cime a torzsadatban');

    // a cim lenyilo projektnel is kinal
    const html = c.V41OutlookImport.supplierAddressSelect({ ...entry, pickupAddress: '' });
    const rows = [...html.matchAll(/<option value="([^"]*)">([^<]*)<\/option>/g)];
    assert.ok(rows.length >= 1, 'projekt felrakonal ures a cim lista');
    assert.match(rows[0][1], /Kazinczy/, 'nem a projekt cimet kinalja');
  });

  await test('Visszarunal a felrako cime a korabbi fuvarokbol potlodik', async () => {
    const c = createContext();
    // a projektnek NINCS cime a torzsadatban - a 60-bol 36 ilyen
    c.state.projects = c.state.projects.map(p2 =>
      /Moxy_bérlemények/.test(p2.name) ? { ...p2, address: '' } : p2);
    c.state.orders = [
      { projectName: 'Budapest_Moxy_bérlemények', dropAddress: '1075 Budapest, Kazinczy u. 48.' },
      { projectName: 'Budapest_Moxy_bérlemények', dropAddress: '1075 Budapest, Kazinczy u. 48.' },
      { projectName: 'Budapest_Moxy_bérlemények', dropAddress: '1061 Budapest, Király utca 12.' }];

    const known = c.V41OutlookImport.projectAddressFromHistoryV90('Budapest_Moxy_bérlemények');
    assert.equal(known, '1075 Budapest, Kazinczy u. 48.', 'nem a leggyakoribb cimet adja: ' + known);
    assert.equal(c.V41OutlookImport.projectAddressFromHistoryV90('Nincs ilyen projekt'), '',
      'ismeretlen projekthez is kitalal cimet');
    assert.equal(c.V41OutlookImport.projectAddressFromHistoryV90(''), '', 'ures nevre is ad cimet');
  });

  await test('A visszaru import megforditja az iranyt es kitolti a cimeket', async () => {
    const c = createContext();
    c.state.orders = [{ projectName: 'Budapest_Moxy_bérlemények', dropAddress: '1075 Budapest, Kazinczy u. 48.' }];
    const entry = c.V41OutlookImport.buildExtractedEntry({
      category: 'megrendeles', sourceName: 'Re_ Megrendelés_Moxy_bérlemény_260921_006327_mez.msg',
      subject: 'Re: Megrendelés_Moxy_bérlemény_260921_006327_mez',
      body: 'Szia! Ezt a visszárut vissza tudjuk vinni a Lambdának a Hengermalomba.',
      pdfName: '', pdfText: '', pdfLines: [], pdfRows: [], attachmentNames: [] });

    assert.equal(entry.orderType, 'VISSZARU', 'nem ismerte fel visszarunak');
    assert.equal(entry.isReturn, true, 'nincs visszaru jelolve');
    // az irany megfordul: a felrako projekt, a lerako beszallito
    assert.ok(entry.pickupAddress, 'a felrako cime URES maradt');
    assert.match(entry.projectName, /Lambda/, 'a lerako nem a beszallito');
    assert.match(entry.dropAddress, /Hengermalom/, 'a lerako cime nem a telephely');
  });

  await test('Egy rendeleshez tobb felrako is tartozhat', async () => {
    const c = createContext();
    const P = c.V41OutlookImport;
    const key = (name, address) => P.pickupIdentityKey(name, address);
    c.state.orders = [{ id: 'a', scheduleDate: '2026-09-23', orderNo: '006327', outlookImport: true,
      orderType: 'SR0', pickupName: 'Lambda Systeme Kft.', pickupAddress: '1117 Budapest, Hengermalom út 47A' }];

    // ugyanarrol a felrakorol: FRISSITES
    assert.equal(P.matchingOutlookOrders(['006327'], 'SR0', false, '2026-09-23',
      key('Lambda Systeme Kft.', '1117 Budapest, Hengermalom út 47A')).length, 1,
      'ugyanarrol a felrakorol nem frissit');

    // masik telephelyrol: UJ FUVAR, a regi marad
    assert.equal(P.matchingOutlookOrders(['006327'], 'SR0', false, '2026-09-23',
      key('Lambda Systeme Kft.', '1106 Budapest, Akna u. 2-4.')).length, 0,
      'masik telephelyrol is felulirja a korabbit');

    // masik cegtol: UJ FUVAR
    assert.equal(P.matchingOutlookOrders(['006327'], 'SR0', false, '2026-09-23',
      key('Gienger Hungária Kft', '1225 Budapest, Dűlő utca 31-35.')).length, 0,
      'masik cegtol is felulirja a korabbit');
  });

  await test('A felrako kulcsa turi a cim irasmodjat', async () => {
    const c = createContext();
    const P = c.V41OutlookImport;
    // 47/a és 47A ugyanaz a telephely
    assert.equal(P.pickupIdentityKey('Lambda', '1117 Budapest, Hengermalom út 47/a'),
      P.pickupIdentityKey('Lambda', '1117 Budapest, Hengermalom út 47A'),
      'a hazszam irasmodja kulon telephelynek szamit');
    // kulonbozo telephely viszont kulon kulcs
    assert.notEqual(P.pickupIdentityKey('Lambda', 'Hengermalom út 47A'),
      P.pickupIdentityKey('Lambda', 'Akna u. 2-4.'), 'ket telephely azonos kulcsot kapott');
  });

  await test('A csere a felrakot is figyelembe veszi', async () => {
    const v41 = fs.readFileSync(__dirname + '/planner-v41.js', 'utf8');
    assert.ok(v41.includes('function pickupIdentityKey'), 'nincs felrako-kulcs');
    const start = v41.indexOf('const replaceIds = new Set();');
    const body = v41.slice(start, start + 800);
    assert.ok(body.includes('pickupIdentityKey(entry.pickupName, entry.pickupAddress)'),
      'a csere nem nezi a felrakot');
  });

  await test('Harom felrako ugyanahhoz a rendeleshez (006306)', async () => {
    /* Valos eset: egy rendelesszam, harom Merkapt telephely. A level maga
       bontja szet ("Merkapt Maglodi ut", "Merkapt Robert Karoly", "Merkapt
       Mester utca"). Ujraimportalaskor mindharom kulon fuvar kell legyen. */
    const c = createContext();
    const P = c.V41OutlookImport;
    const sites = [
      '1106 Budapest, Maglódi út 14B',
      '1134 Budapest, Róbert Károly krt. 64-66.',
      '2143 Kistarcsa, Mester utca 5.'];

    c.state.orders = [];
    for (const address of sites) {
      const key = P.pickupIdentityKey('Merkapt Zrt.', address);
      const hits = P.matchingOutlookOrders(['006306'], 'SR0', false, '2026-09-23', key);
      assert.equal(hits.length, 0, 'mas telephelyet talalt egyezesnek: ' + address);
      c.state.orders.push({ id: 'o' + c.state.orders.length, scheduleDate: '2026-09-23',
        orderNo: '006306', outlookImport: true, orderType: 'SR0',
        pickupName: 'Merkapt Zrt.', pickupAddress: address });
    }
    assert.equal(c.state.orders.length, 3, 'nem harom kulon fuvar keletkezett');

    // a masodik import UGYANARROL a telephelyrol mar frissit, nem szaporit
    const again = P.matchingOutlookOrders(['006306'], 'SR0', false, '2026-09-23',
      P.pickupIdentityKey('Merkapt Zrt.', sites[0]));
    assert.equal(again.length, 1, 'ugyanarrol a telephelyrol nem frissit');
  });

  await test('A kezi szerkesztes nem hoz letre uj fuvart', async () => {
    /* A buborekban atirt cim csak MODOSIT - az import logikaja oda nem fut be.
       A megkulonboztetes a felrako kulcsan mulik, amit csak a commitImport
       hasznal. */
    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    const start = app.indexOf("$('#orderForm').onsubmit");
    const body = app.slice(start, app.indexOf('\n', start));
    assert.ok(!body.includes('matchingOutlookOrders'), 'a kezi mentes is cserelogikat futtat');
    assert.ok(!body.includes('pickupIdentityKey'), 'a kezi mentes a felrako-kulcsot hasznalja');
  });

  function geoCtx() {
    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    const c = { console: { info() {} }, String, Array, Object, RegExp, Error, Map, Set };
    c.globalThis = c; c.window = c;
    vm.createContext(c);
    const i = app.indexOf('function retryApproxGeoV92');
    vm.runInContext(app.slice(i, app.indexOf('async function geo(addr')), c, { filename: 'geo' });
    return c;
  }

  await test('A cimet tobbfele alakban keressuk', async () => {
    const c = geoCtx();
    const variants = c.addressVariantsV92('1106 Budapest, Maglódi út 14/B');
    assert.ok(variants.includes('1106 Budapest, Maglódi út 14/B'), 'hianyzik a nyers alak');
    assert.ok(variants.includes('1106 Budapest, Maglódi út 14B'), 'hianyzik az egybeirt hazszam');
    assert.ok(variants.includes('1106 Budapest, Maglódi út 14'), 'hianyzik a betujel nelkuli alak');
    assert.ok(variants.some(v => /Maglódi út$/.test(v)), 'hianyzik a hazszam nelkuli alak');
    // forditva is: 14B -> 14/B
    assert.ok(c.addressVariantsV92('Maglódi út 14B').includes('Maglódi út 14/B'),
      'az egybeirtbol nem keszul per-jeles alak');
    // tartomany elso tagja
    assert.ok(c.addressVariantsV92('Akna u. 2-4.').some(v => /2\./.test(v)), 'a tartomany nem bomlik');
  });

  await test('A strukturalt kereses szetszedi a cimet', async () => {
    const c = geoCtx();
    const q = c.structuredQueryV92('1106 Budapest, Maglódi út 14B');
    assert.equal(q.zip, '1106', 'nem ismerte fel az iranyitoszamot');
    assert.equal(q.city, 'Budapest', 'nem ismerte fel a varost');
    assert.match(q.street, /Maglódi út 14B/, 'nem ismerte fel az utcat: ' + q.street);
    const k = c.structuredQueryV92('2143 Kistarcsa, Mester utca 5.');
    assert.equal(k.city, 'Kistarcsa');
    assert.equal(k.zip, '2143');
  });

  await test('A becsult pontok ujrakereshetove valnak', async () => {
    const c = geoCtx();
    c.state = { geo: {
      'Maglódi út 14B': [47.483, 19.145], 'Akna u. 2-4.': [47.483, 19.145], 'Kőér u. 3B': [47.483, 19.145],
      'Pontos cím 1.': [47.5001, 19.0501], 'Kézi cím': [47.6, 19.2] },
      geoSources: { 'Pontos cím 1.': 'nominatim', 'Kézi cím': 'manual' }, geoApprox: {} };
    const released = c.retryApproxGeoV92();
    assert.equal(released, 3, 'nem a harom osztott pontot engedte el: ' + released);
    assert.ok(c.state.geo['Pontos cím 1.'], 'a pontos talalatot is eldobta');
    assert.ok(c.state.geo['Kézi cím'], 'a kezzel megadott koordinatat is eldobta');
    assert.ok(!c.state.geo['Maglódi út 14B'], 'a becsult pont megmaradt');
  });

  await test('Az uj import a lista tetejere kerul', async () => {
    const v41 = fs.readFileSync(__dirname + '/planner-v41.js', 'utf8');
    assert.ok(v41.includes('pending.unshift(...entries)'), 'az uj rendeles a lista aljara kerul');
    assert.ok(!/pending\.push\(\.\.\.entries\)/.test(v41), 'maradt a regi, vegere fuzo beszuras');
    // hibas fajl eseten is elol
    assert.ok(v41.includes('pending.unshift({'), 'a hibas fajl kartyaja a vegere kerul');
    const html = fs.readFileSync(__dirname + '/index.html', 'utf8');
    assert.ok(html.includes('A legutóbb behúzott rendelés legfelül'), 'a felirat nem tukrozi a sorrendet');
  });

  await test('Atrendezes utan a terkep szamozasa is frissul', async () => {
    /* A tervfrissites eddig csak azt nezte, ugyanazok a fuvarok vannak-e a
       tervben, es stimmel-e a cimuk - a SORRENDJUKET nem. Ezert a Nezetben
       vegzett atrendezes utan a terkepen a regi szamozas maradt. */
    const c = createContext();
    const P = c.V96Planner || c.V65Planner;
    c.state.vehicles = [{ id: 'm', driverName: 'Márió', active: true }];
    c.state.orders = [
      { id: 'a', scheduleDate: '2026-09-01', vehicleId: 'm', sequence: 1, pickupName: 'A', pickupAddress: 'Alfa utca 1.', projectName: 'P', items: [] },
      { id: 'b', scheduleDate: '2026-09-01', vehicleId: 'm', sequence: 2, pickupName: 'B', pickupAddress: 'Béta utca 2.', projectName: 'P', items: [] },
      { id: 'c', scheduleDate: '2026-09-01', vehicleId: 'm', sequence: 3, pickupName: 'C', pickupAddress: 'Gamma utca 3.', projectName: 'P', items: [] }];
    const plan = [
      { type: 'pickup', name: 'A', address: 'Alfa utca 1.', orders: ['a'] },
      { type: 'pickup', name: 'B', address: 'Béta utca 2.', orders: ['b'] },
      { type: 'pickup', name: 'C', address: 'Gamma utca 3.', orders: ['c'] }];

    assert.equal(P.routePlanNeedsRefreshV55('m', plan), false,
      'valtozatlan sorrendnel is ujrarajzolna');

    // a felhasznalo atrendezi: C kerul elore
    c.state.orders[2].sequence = 1;
    c.state.orders[0].sequence = 2;
    c.state.orders[1].sequence = 3;
    assert.equal(P.routePlanNeedsRefreshV55('m', plan), true,
      'atrendezes utan is frissnek tartja a regi tervet');
  });

  await test('A sorrend-ellenorzes bekerult a tervfrissitesbe', async () => {
    const v44 = fs.readFileSync(__dirname + '/planner-v44.js', 'utf8');
    const start = v44.indexOf('function routePlanNeedsRefreshV55');
    const body = v44.slice(start, v44.indexOf('\n  }', v44.indexOf('planOrder', start)));
    assert.ok(body.includes('const planOrder ='), 'nincs sorrend-ellenorzes');
    assert.match(body, /if \(planOrder\[i\] < planOrder\[i - 1\]\) return true;/,
      'nem veti ossze a terv sorrendjet a sorszamokkal');
  });

  function dailyListCtx(orders, vehicles) {
    const src = fs.readFileSync(__dirname + '/daily-list-v93.js', 'utf8');
    const c = { console, String, Object, Array, RegExp, Error, Math, Number, Boolean, setTimeout: f => f() };
    c.globalThis = c; c.window = c;
    c.board = { style: {}, innerHTML: '' };
    c.dateLabel = { textContent: '' };
    c.document = {
      querySelector: sel => sel === '#v93Board' ? c.board : (sel === '#v94Date' ? c.dateLabel : null),
      querySelectorAll: () => [],
      getElementById: () => null };
    c.selectedDate = () => '2026-09-24';
    c.v24DriverKey = v => (v.driverName || '').toLowerCase().replace(/[^a-z]/g, '');
    c.activeVehicles = () => vehicles;
    c.dayOrders = id => orders.filter(o => o.vehicleId === id);
    c.isResolvedBacklogOrder = () => false;
    c.sortableCalls = 0;
    c.initSortables = () => { c.sortableCalls++; };
    vm.createContext(c);
    vm.runInContext(src, c, { filename: 'daily' });
    return c;
  }

  const V93_VEHICLES = [
    { id: 'm', driverName: 'Márió', name: 'Dobozos' },
    { id: 'p', driverName: 'Patrik', name: 'Dobozos' },
    { id: 't', driverName: 'Martin', name: 'Platós' }];
  const V93_ORDERS = [
    { id: '1', vehicleId: 'm', pickupName: 'Airvent Légtechnikai Zrt', pickupAddress: '1103 Budapest, Kőér u. 3B', sequence: 1, items: [1, 2, 3] },
    { id: '2', vehicleId: 'm', pickupName: 'Lambda Systeme Kft.', pickupAddress: '1117 Budapest, Hengermalom út 47A', sequence: 2, items: [1] },
    { id: '3', vehicleId: 'm', pickupName: 'Hydrotrade Kft', pickupAddress: '2045 Törökbálint', sequence: 3, items: [1], completed: true },
    { id: '4', vehicleId: 'p', pickupName: 'EZER-KER Kft.', pickupAddress: '1106 Budapest, Kada utca 149', sequence: 1, items: [1, 2] },
    { id: '5', vehicleId: 't', pickupName: 'Merkapt Zrt.', pickupAddress: '1106 Budapest, Maglódi út 14B', sequence: 1, items: [1] }];

  await test('A napi lista minden sofor oszlopat kirajzolja', async () => {
    const c = dailyListCtx(V93_ORDERS, V93_VEHICLES);
    c.renderDailyListV93();
    const html = c.board.innerHTML;
    assert.equal((html.match(/class="v93-col"/g) || []).length, 3, 'nem harom oszlop');
    assert.equal((html.match(/class="v93-row/g) || []).length, 5, 'nem minden fuvar jelent meg');
    assert.equal(c.board.style.gridTemplateColumns, 'repeat(3, minmax(0, 1fr))', 'rossz oszlopszelesseg');
    assert.equal(c.dateLabel.textContent, '2026-09-24', 'nem irta ki a napot');
    // a fejlecek osszesitenek
    // V96: felrakonkent egy sor, a fejlec felrakot/rendelest/tetelt osszesit
    assert.match(html, /felrakó · .* rendelés · .* tétel/, 'rossz a fejlec osszesitese');
  });

  await test('A huzas ugyanazt a mozgatologikat hasznalja', async () => {
    const c = dailyListCtx(V93_ORDERS, V93_VEHICLES);
    c.renderDailyListV93();
    const html = c.board.innerHTML;
    // a konteneren ugyanaz az azonosito, amit az initSortables keres
    // V94: sajat azonosito, mert a "route-<id>" utkozott a fooldallal
    for (const id of ['v94-route-m', 'v94-route-p', 'v94-route-t']) {
      assert.ok(html.includes(`id="${id}"`), 'hianyzik a mozgatokontener: ' + id);
    }
    assert.ok(html.includes('class="v96-grip"'), 'a sorszam nem fogopont');
    assert.ok(!html.includes('ide húzhatod'), 'maradt a szaggatott ejtosav');
  });

  await test('A kesz fuvar jelolve van es nem mozgathato', async () => {
    const c = dailyListCtx(V93_ORDERS, V93_VEHICLES);
    c.renderDailyListV93();
    const html = c.board.innerHTML;
    assert.ok(html.includes('v93-done'), 'a kesz fuvar nincs megjelolve');
    // V94: a kesz fuvar szurke, de MOZGATHATO - utolag is javithato, ki vitte el
    assert.match(html, /Kész · /, 'nincs jelolve a kesz fuvar');
    const doneRow = html.slice(html.indexOf('v93-done'));
    assert.ok(doneRow.includes('v96-grip'), 'a kesz fuvar nem mozgathato');
  });

  await test('A napi lista be van kotve a feluletbe', async () => {
    const html = fs.readFileSync(__dirname + '/index.html', 'utf8');
    assert.ok(html.includes('data-page="dailylist"'), 'nincs oldalsavi gomb');
    assert.ok(html.includes('id="dailylist"'), 'nincs meg a lap');
    assert.ok(html.includes('daily-list-v93.js'), 'a script nincs betoltve');
    // a Fuvarok UTAN all a gomb
    assert.ok(html.indexOf('data-page="planner"') < html.indexOf('data-page="dailylist"'),
      'a gomb nem a Fuvarok utan van');

    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    assert.ok(app.includes("id==='dailylist'&&typeof renderDailyListV93"), 'lapvaltaskor nem rajzol');
    assert.ok(app.includes('renderDailyListV93,50'), 'a fooldali frissites nem koveti');

    const v37 = fs.readFileSync(__dirname + '/planner-v37.js', 'utf8');
    assert.ok(v37.includes('renderDailyListV93'), 'huzas utan nem frissul a napi lista');
  });

  await test('A napi lista sajat mozgatast kap, nem utkozik a fooldallal', async () => {
    const js = fs.readFileSync(__dirname + '/daily-list-v93.js', 'utf8');
    assert.ok(js.includes("'v94-route-' + vehicleId"), 'nem sajat a kontener-azonosito');
    assert.ok(!/id="route-\$\{/.test(js), 'maradt a fooldallal utkozo azonosito');
    assert.ok(js.includes("group: 'v94-daily'"), 'nincs sajat mozgatocsoport');
    assert.ok(js.includes("handle: '.v96-grip'"), 'nem a sorszam a fogopont');
    // a fooldali initSortables NEM ezeket keresi
    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    assert.ok(app.includes("$('#route-'+v.id)"), 'megvaltozott a fooldali bekotes');
  });

  await test('A naplepteto ugyanazt a napot allitja, mint a fooldal', async () => {
    const js = fs.readFileSync(__dirname + '/daily-list-v93.js', 'utf8');
    assert.ok(js.includes("function stepDayV94"), 'nincs naplepteto');
    assert.ok(js.includes("$('#workDate')"), 'nem a fooldali datummezot allitja');
    assert.ok(js.includes('shiftWorkday'), 'nem munkanapot lepteti');
    const html = fs.readFileSync(__dirname + '/index.html', 'utf8');
    for (const id of ['v94Prev', 'v94Next', 'v94Picker', 'v94Date']) {
      assert.ok(html.includes(`id="${id}"`), 'hianyzik a kezeloelem: ' + id);
    }
    const app = fs.readFileSync(__dirname + '/app.js', 'utf8');
    assert.ok(app.includes('bindDayControlsV94'), 'a lepteto nincs bekotve lapvaltaskor');
  });

  await test('MINDEN fogopont a soraban marad, nem ugrik ki', async () => {
    /* A globalis .drag szabaly position:absolute-ot ad (a regi buborekokban ez
       a helyes). Barmely UJ sor, ami a .drag osztalyt hasznalja fogopontnak,
       kiugrik a helyerol, es NEM LEHET MEGFOGNI - a huzas el sem indul.

       Ez mar ketszer megtortent: a Nezet soraval (V83) es a napi fuvarlistaval
       (V95). Ez a teszt minden ilyen sorra figyel. */
    const css = fs.readFileSync(__dirname + '/styles.css', 'utf8');
    assert.match(css, /\.drag\s*\{[^}]*position:absolute/,
      'eltunt a globalis .drag szabaly - a regi buborekok fogantyuja elromolhat');

    // minden sorosztaly, ami .drag fogopontot hasznal, kapjon visszaallitast
    const rowClasses = ['v56-row'];
    for (const cls of rowClasses) {
      const rule = css.match(new RegExp('\\.' + cls + ' \\.v56-index\\.drag\\{([^}]*)\\}'));
      assert.ok(rule, cls + ': nincs fogopont-visszaallitas');
      assert.match(rule[1], /position:static!important/, cls + ': a fogopont kiugrik a sorbol');
      assert.match(rule[1], /display:flex!important/, cls + ': a fogopont rejtve maradhat');
      assert.match(rule[1], /cursor:grab/, cls + ': a fogopont nem jelzi, hogy foghato');
      const size = rule[1].match(/width:(\d+)px;height:(\d+)px/);
      assert.ok(size && +size[1] >= 28, cls + ': a fogopont tul kicsi ujjal');
    }
  });

  await test('A napi lista sorai ujjal is foghatok', async () => {
    const css = fs.readFileSync(__dirname + '/styles.css', 'utf8');
    const rule = css.match(/\.v96-grip\{([^}]*)\}/);
    assert.match(rule[1], /touch-action:none/, 'erintokepernyon a gorgetes viszi el a huzast');
    assert.match(rule[1], /user-select:none/, 'huzas kozben szoveget jelolne ki');
  });

  function groupCtx(orders, vehicles) {
    const src = fs.readFileSync(__dirname + '/daily-list-v93.js', 'utf8');
    const c = { console, String, Object, Array, RegExp, Error, Math, Number, Boolean, Map, Set, setTimeout: f => f() };
    c.globalThis = c; c.window = c;
    const nodes = {};
    const mk = id => (nodes['#' + id] = { id, dataset: {}, innerHTML: '', style: {}, rows: [] });
    mk('v93Board'); nodes['#v94Date'] = { textContent: '' };
    vehicles.forEach(v => mk('v94-route-' + v.id));
    c.nodes = nodes;
    c.document = {
      querySelector: sel => nodes[sel] || null,
      querySelectorAll: sel => {
        const m = /^#(v94-route-[a-z0-9]+) \.v93-row$/.exec(sel);
        return m && nodes['#' + m[1]] ? nodes['#' + m[1]].rows : [];
      },
      getElementById: id => nodes['#' + id] || null };
    c.selectedDate = () => '2026-09-25';
    c.v24DriverKey = v => (v.driverName || '').toLowerCase().replace(/[^a-z]/g, '');
    c.activeVehicles = () => vehicles;
    c.state = { orders };
    c.dayOrders = id => orders.filter(o => o.vehicleId === id);
    c.isResolvedBacklogOrder = () => false;
    c.save = () => {}; c.renderRoutes = () => {}; c.render = () => {};
    c.bound = [];
    c.Sortable = function (el, opt) { c.bound.push({ el, opt }); };
    vm.createContext(c);
    vm.runInContext(src, c, { filename: 'daily' });
    return c;
  }

  await test('Minden felrako CSAK EGYSZER szerepel a napi listaban', async () => {
    const orders = [];
    let n = 0;
    const add = (v, name, address, items) => orders.push({ id: 'o' + (++n), vehicleId: v,
      pickupName: name, pickupAddress: address, sequence: n, items: Array(items).fill(1) });
    add('m', 'Merkapt Zrt.', '1106 Budapest, Maglódi út 14/B', 6);
    add('m', 'Merkapt Zrt.', '1106 Budapest, Maglódi út 14/B', 4);
    add('m', 'Merkapt Zrt.', '1106 Budapest, Maglódi út 14/B', 2);
    add('m', 'EZER-KER Kft.', '1106 Budapest, Kada utca. 149.', 1);
    add('m', 'EZER-KER Kft.', '1106 Budapest, Kada utca. 149.', 3);

    const c = groupCtx(orders, [{ id: 'm', driverName: 'Márió' }]);
    c.renderDailyListV93();
    const html = c.nodes['#v93Board'].innerHTML;
    assert.equal((html.match(/class="v93-row/g) || []).length, 2,
      '5 rendelesbol nem 2 felrako-sor lett');
    assert.match(html, /3 rendelés · 12 tétel/, 'nem osszesiti a Merkapt rendeleseit');
    assert.match(html, /2 rendelés · 4 tétel/, 'nem osszesiti az EZER-KER rendeleseit');
    assert.match(html, /2 felrakó · 5 rendelés · 16 tétel/, 'rossz a fejlec osszesitese');
  });

  await test('A felrako sorat huzva MINDEN rendelese egyutt mozog', async () => {
    const orders = [
      { id: 'a1', vehicleId: 'm', pickupName: 'Merkapt', pickupAddress: 'Maglódi 14/B', sequence: 1, items: [1] },
      { id: 'a2', vehicleId: 'm', pickupName: 'Merkapt', pickupAddress: 'Maglódi 14/B', sequence: 2, items: [1] },
      { id: 'b1', vehicleId: 'm', pickupName: 'EZER-KER', pickupAddress: 'Kada 149', sequence: 3, items: [1] },
      { id: 'c1', vehicleId: 't', pickupName: 'Lambda', pickupAddress: 'Hengermalom 47A', sequence: 1, items: [1] }];
    const vehicles = [{ id: 'm', driverName: 'Márió' }, { id: 't', driverName: 'Martin' }];
    const c = groupCtx(orders, vehicles);
    c.renderDailyListV93();
    assert.equal(c.bound[0].opt.handle, '.v96-grip', 'nem a sajat fogopont indit huzast');

    const mParent = { children: [] }, tParent = { children: [] };
    const row = (ids, parent) => ({ dataset: { ids }, parentElement: parent,
      querySelector: () => ({ textContent: '' }) });
    const rowB = row('b1', mParent); mParent.children = [rowB];
    const rowC = row('c1', tParent), rowA = row('a1,a2', tParent);
    tParent.children = [rowC, rowA];
    c.nodes['#v94-route-m'].rows = [rowB];
    c.nodes['#v94-route-t'].rows = [rowC, rowA];
    c.bound[0].opt.onEnd({ item: { dataset: { ids: 'a1,a2' } }, to: { id: 'v94-route-t' } });

    assert.equal(orders.find(o => o.id === 'a1').vehicleId, 't', 'az elso rendeles nem mozdult');
    assert.equal(orders.find(o => o.id === 'a2').vehicleId, 't', 'a masodik rendeles ottmaradt');
    assert.equal(orders.find(o => o.id === 'b1').vehicleId, 'm', 'mas felrakot is elvitt');
    // folytonos sorszamozas mindket sofornél
    assert.equal(orders.find(o => o.id === 'b1').sequence, 1, 'a forras oszlop nem szamozodott ujra');
    const martin = orders.filter(o => o.vehicleId === 't').map(o => o.sequence).sort();
    assert.deepEqual(martin, [1, 2, 3], 'nem folytonos a sorszamozas: ' + martin.join(','));
  });

  await test('A fogopont sajat osztalyt visel, nem a regi .drag-et', async () => {
    const js = fs.readFileSync(__dirname + '/daily-list-v93.js', 'utf8');
    assert.ok(js.includes('class="v96-grip"'), 'nincs sajat fogopont-osztaly');
    assert.ok(!js.includes('v56-index drag'), 'maradt a regi, utkozo osztalynev');
    const css = fs.readFileSync(__dirname + '/styles.css', 'utf8');
    const rule = css.match(/\.v96-grip\{([^}]*)\}/);
    assert.ok(rule, 'nincs stilusa a fogopontnak');
    assert.ok(!/position:absolute/.test(rule[1]), 'a fogopont kiugrik a sorbol');
    assert.match(rule[1], /cursor:grab/, 'nem jelzi, hogy foghato');
  });

  if (!process.exitCode) console.log(`\nV96 elfogadási teszt: ${passed}/${total} sikeres.`);
})();
