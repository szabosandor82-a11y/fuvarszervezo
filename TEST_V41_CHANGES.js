...
await test('ugyanarrol a felrakorol frissit, masikrol UJ fuvart hoz letre',()=>{const e0=baseEntry();state.orders=[{id:'old',outlookImport:true,orderType:'SR0',isReturn:false,orderNo:'004937',sourceOrderNos:['004937'],scheduleDate:'2026-08-05',pickupName:e0.pickupName,pickupAddress:e0.pickupAddress,projectName:'Budapest_Cosmo_Residence'}];const e=baseEntry();e.scheduleDate='2026-08-05';V.setPending([e]);V.commitImport();eq(state.orders.filter(o=>o.orderNo==='004937').length,1,'ugyanarrol a felrakorol nem frissitett');eq(state.orders.some(o=>o.id==='old'),false,'a regi fuvar megmaradt frissiteskor');state.orders=[{id:'old2',outlookImport:true,orderType:'SR0',isReturn:false,orderNo:'004937',sourceOrderNos:['004937'],scheduleDate:'2026-08-05',pickupName:'Merkapt Zrt.',pickupAddress:'1108 Budapest, Maglódi utca 123',projectName:'Budapest_Cosmo_Residence'}];const e2=baseEntry();e2.scheduleDate='2026-08-05';V.setPending([e2]);V.commitImport();eq(state.orders.filter(o=>o.orderNo==='004937').length,2,'masik felrakorol nem keletkezett uj fuvar');eq(state.orders.some(o=>o.id==='old2'),true,'a masik felrakos fuvart torolte');});
await test('Outlook előnézet ürítése a már mentett fuvarokat és hátralékot nem törli',()=>{state.orders=[{id:'out',outlookImport:true,orderNo:'1'},{id:'manual',outlookImport:false,orderNo:'2'}];state.backlog=[{id:'b',targetOrderId:'out'}];V.setPending([baseEntry()]);V.clearAllImports();eq(state.orders.map(o=>o.id),['out','manual']);eq(state.backlog.length,1);eq(V.getPending().length,0)});
