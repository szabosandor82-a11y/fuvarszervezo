...
(function (global) {
  ...
  async function getSettings() {
    const rows = await dbRequest('app_settings?select=id,delivery_email,updated_at&id=eq.1');
    return rows?.[0] || { id: 1, delivery_email: config.deliveryEmailDefault || 'szabo.sandor@stand98' };
  }
  async function updateSettings(settings) {
    const body = { id: 1, delivery_email: String(settings.delivery_email || '').trim(), updated_at: new Date().toISOString() };
    const rows = await dbRequest('app_settings?on_conflict=id', { method: 'POST', body, headers: { Prefer: 'resolution=merge-duplicates,return=representation' } });
    return rows?.[0] || body;
  }
  ...
  async function createDeliveryReport(order, files, note = '', audio = null) {
    ...
    try {
      emailResult = await rawRequest(`${baseUrl()}/functions/v1/send-delivery-report`, { method: 'POST', body: { reportId: report.id } });
    } catch (error) {
      emailResult = { ok: false, error: error.message };
    }
    return { report: { ...report, fileCount: fileRows.length }, files: fileRows, emailResult };
  }
  ...
  async function invokeEmail(reportId) {
    return rawRequest(`${baseUrl()}/functions/v1/send-delivery-report`, { method: 'POST', body: { reportId } });
  }
  ...
  global.V44Online = {
    ...
    getSettings, updateSettings, createDeliveryReport, listDeliveryFiles, invokeEmail,
