...
global.V40PdfTextExtractor=async bytes=>{
  const tmp=path.join(os.tmpdir(),`v40-${process.pid}-${Math.random().toString(16).slice(2)}.pdf`);
...
vm.runInThisContext(fs.readFileSync('./planner-v40.js','utf8'),{filename:'planner-v40.js'});
const V=global.V40OutlookImport;
...
await test('MSG végponttól végpontig: KRPR 000745',async()=>{const [e]=await V.parseDroppedFile(fileObj('/mnt/data/Raktárközi_Lejardin_260804_000745_gumikomp.msg'),'dobozos');eq([e.orderNo,e.orderType,e.pickupAddress,e.projectName,e.items.length],['000745','KRPR','2310 Szigetszentmiklós, Kereskedő utca 2.','Budapest_LeJardin_II_felépítmény',1])});
...
console.log(`\nV40 teljes MSG teszt: ${pass}/${pass+fail} sikeres.`); if(fail)process.exit(1);
})();
