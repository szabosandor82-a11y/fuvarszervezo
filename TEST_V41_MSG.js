...
await test('MSG végponttól végpontig: City Pearl 004932 – ismeretlen beszállító üresen marad',async()=>{const [e]=await V.parseDroppedFile(fileObj('/mnt/data/Megrendelés_City Pearl_II_260804_004932_PVC-U_golyóscsap(1).msg'),'dobozos');eq([e.orderNo,e.pickupName,e.pickupAddress,e.supplierId,e.projectName,e.dropAddress,e.items.length],['004932','','','','Budapest_City_pearl_II.ütem','1095 Budapest, Soroksári út 58.',1]);eq(e.newSupplierData,null);ok(e.warnings.some(w=>/beszállítói törzsadatokban/.test(w)))});
