...
(function (global) {
  ...
  const id = () => typeof uid === 'function' ? uid() : `v40-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  ...
  async function parseMsgFile(file, category) {
    ...
    for (const attachment of attachments) {
      ...
      try {
        ...
        console.warn('[V40] PDF melléklet hiba', attachmentName, error);
      }
    }
  ...
  async function processFiles(fileList, category) {
    ...
    for (let index = 0; index < files.length; index++) {
      ...
      try {
        ...
        failures++;
        console.error('[V40] Outlook import hiba', file.name, error);
        pending.push({
