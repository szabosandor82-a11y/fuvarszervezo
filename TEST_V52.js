...
passed += ok('A szétosztómotor kiadási verziója V70', () => {
  assert.match(index, /Fuvarszervező V96/);
  assert.match(app, /APP_VERSION=/);
  assert.match(planner, /const VERSION = '96'/);
  assert.equal(manifest.name, 'Fuvarszervező V96');
  assert.equal(manifest.short_name, 'Fuvar V96');
  assert.match(sw, /fuvarszervezo-v96-online-20260925-1/);
  assert.ok(!index.includes('?v=51.0'));
  assert.match(index, /\?v=96\.0/);
});
