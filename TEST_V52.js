...
passed += ok('A szétosztómotor kiadási verziója V70', () => {
  assert.match(index, /Fuvarszervező V85/);
  assert.match(app, /APP_VERSION=/);
  assert.match(planner, /const VERSION = '85'/);
  assert.equal(manifest.name, 'Fuvarszervező V85');
  assert.equal(manifest.short_name, 'Fuvar V85');
  assert.match(sw, /fuvarszervezo-v85-online-20260923-2/);
  assert.ok(!index.includes('?v=51.0'));
  assert.match(index, /\?v=85\.0/);
});
