...
passed += ok('A szétosztómotor kiadási verziója V70', () => {
  assert.match(index, /Fuvarszervező V88/);
  assert.match(app, /APP_VERSION=/);
  assert.match(planner, /const VERSION = '88'/);
  assert.equal(manifest.name, 'Fuvarszervező V88');
  assert.equal(manifest.short_name, 'Fuvar V88');
  assert.match(sw, /fuvarszervezo-v88-online-20260923-5/);
  assert.ok(!index.includes('?v=51.0'));
  assert.match(index, /\?v=88\.0/);
});
