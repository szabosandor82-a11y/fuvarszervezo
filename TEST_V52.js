...
passed += ok('A szétosztómotor kiadási verziója V70', () => {
  assert.match(index, /Fuvarszervező V90/);
  assert.match(app, /APP_VERSION=/);
  assert.match(planner, /const VERSION = '90'/);
  assert.equal(manifest.name, 'Fuvarszervező V90');
  assert.equal(manifest.short_name, 'Fuvar V90');
  assert.match(sw, /fuvarszervezo-v90-online-20260923-7/);
  assert.ok(!index.includes('?v=51.0'));
  assert.match(index, /\?v=90\.0/);
});
