...
passed += ok('A szétosztómotor kiadási verziója V70', () => {
  assert.match(index, /Fuvarszervező V95/);
  assert.match(app, /APP_VERSION=/);
  assert.match(planner, /const VERSION = '95'/);
  assert.equal(manifest.name, 'Fuvarszervező V95');
  assert.equal(manifest.short_name, 'Fuvar V95');
  assert.match(sw, /fuvarszervezo-v95-online-20260924-4/);
  assert.ok(!index.includes('?v=51.0'));
  assert.match(index, /\?v=95\.0/);
});
