...
passed += ok('A szétosztómotor kiadási verziója V55', () => {
  assert.match(index, /Fuvarszervező V61/);
  assert.match(app, /APP_VERSION=/);
  assert.match(planner, /const VERSION = '61'/);
  assert.equal(manifest.name, 'Fuvarszervező V61');
  assert.equal(manifest.short_name, 'Fuvar V61');
  assert.match(sw, /fuvarszervezo-v61-online-20260828-1/);
  assert.ok(!index.includes('?v=51.0'));
  assert.match(index, /\?v=61\.0/);
});
