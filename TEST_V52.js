...
passed += ok('A szétosztómotor kiadási verziója V70', () => {
  assert.match(index, /Fuvarszervező V78/);
  assert.match(app, /APP_VERSION=/);
  assert.match(planner, /const VERSION = '78'/);
  assert.equal(manifest.name, 'Fuvarszervező V78');
  assert.equal(manifest.short_name, 'Fuvar V78');
  assert.match(sw, /fuvarszervezo-v78-online-20260921-1/);
  assert.ok(!index.includes('?v=51.0'));
  assert.match(index, /\?v=78\.0/);
});
