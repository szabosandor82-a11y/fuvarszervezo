...
passed += ok('A szétosztómotor kiadási verziója V70', () => {
  assert.match(index, /Fuvarszervező V87/);
  assert.match(app, /APP_VERSION=/);
  assert.match(planner, /const VERSION = '87'/);
  assert.equal(manifest.name, 'Fuvarszervező V87');
  assert.equal(manifest.short_name, 'Fuvar V87');
  assert.match(sw, /fuvarszervezo-v87-online-20260923-4/);
  assert.ok(!index.includes('?v=51.0'));
  assert.match(index, /\?v=87\.0/);
});
