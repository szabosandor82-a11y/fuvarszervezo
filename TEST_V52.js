...
passed += ok('A szétosztómotor kiadási verziója V70', () => {
  assert.match(index, /Fuvarszervező V91/);
  assert.match(app, /APP_VERSION=/);
  assert.match(planner, /const VERSION = '91'/);
  assert.equal(manifest.name, 'Fuvarszervező V91');
  assert.equal(manifest.short_name, 'Fuvar V91');
  assert.match(sw, /fuvarszervezo-v91-online-20260923-8/);
  assert.ok(!index.includes('?v=51.0'));
  assert.match(index, /\?v=91\.0/);
});
